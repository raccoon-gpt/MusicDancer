...
import { createStarTunnel } from "./starTunnel.js";
...
const starTunnel = createStarTunnel(container);
// container и camera нужны цветной ударной волне от шара (2D canvas +
// проекция позиции шара на экран, см. createBallShockwave в
// discoLights.js).
const discoLights = createDiscoLights(scene, container, camera);
...
(function setupCharacterColorControls() {
  ...
  window.__updateEdgeBlurCanvas = function updateEdgeBlurCanvas() {
    ...
    // реальный размер после layout-изменения) — drawImage() кидает
    ...
    if (edgeBlurCanvas.width !== renderer.domElement.width || edgeBlurCanvas.height !== renderer.domElement.height) {
      ...
      // кадре, независимо от rAF-throttle в scene.js. Буфер же (строки
      // выше) обновляется только вместе с этим throttle — то есть буфер
      // и видимый размер этого канваса временно расходились при
      // ресайзе (например, кнопка масштаба сцены), давая деформацию
      // именно в этом слое (edge-blur), даже после фикса пропорций
      // камеры в основном WebGL-канвасе. Теперь видимый размер держим
      // синхронно С САМИМ БУФЕРОМ, инлайн-стилем — оба значения всегда
      // меняются вместе, расхождению просто неоткуда взяться. Раньше
      // здесь ещё было отдельное "прятать слой во время активного
      // ресайза" (isResizingScene) — придумано под непрерывный поток
      // событий при перетаскивании хендла шторки списка. Шторка теперь
      // отдельный слой поверх всего (position:fixed) и вообще не трогает
      // размер сцены — того потока событий больше не бывает, а для
      // ОСТАВШИХСЯ одиночных изменений (кнопка масштаба, ресайз окна)
      // прятать/показывать слой только добавляло бы лишнее мигание на
      // ровном месте — эта синхронизация сама по себе справляется чисто,
      // без него.
...
// Девять дефолтных фоновых видео — цикл переключается кнопкой волны (см.
// applyWaveMode ниже). Один переиспользуемый <video>-элемент, src
// подменяется на нужный файл при переключении — не по элементу на
// каждое видео (незачем держать девять параллельных видео в DOM,
// реально показывается только одно за раз).
const DEFAULT_BG_VIDEOS = [
  "assets/bg-videos/11100.mp4",
  "assets/bg-videos/11101.mp4",
  "assets/bg-videos/11102.mp4",
  "assets/bg-videos/11103.mp4",
  "assets/bg-videos/11104.mp4",
  "assets/bg-videos/11105.mp4",
  "assets/bg-videos/11106.mp4",
  "assets/bg-videos/11107.mp4",
  "assets/bg-videos/11108.mp4",
];
const defaultVideoBgEl = document.createElement("video");
defaultVideoBgEl.autoplay = true;
defaultVideoBgEl.loop = true;
defaultVideoBgEl.muted = true;
defaultVideoBgEl.playsInline = true;
defaultVideoBgEl.style.position = "absolute";
defaultVideoBgEl.style.top = "-1px";
defaultVideoBgEl.style.left = "-1px";
defaultVideoBgEl.style.width = "calc(100% + 2px)";
defaultVideoBgEl.style.height = "calc(100% + 2px)";
defaultVideoBgEl.style.objectFit = "cover";
defaultVideoBgEl.style.zIndex = "-1";
defaultVideoBgEl.style.borderRadius = "inherit";
defaultVideoBgEl.style.display = "none"; // изначально скрыто — видно только в своих режимах цикла волны
container.appendChild(defaultVideoBgEl);


const videoBgFileInput = document.createElement("input");
videoBgFileInput.type = "file";
// БЕЗ accept — та же история, что уже была с аудио: accept="video/*,image/*"
// ненадёжно распознаётся системным пикером на части файлов (не все
// .mp4/.jpg с телефона показывались как доступные для выбора), несмотря
// на то что они прекрасно открываются. Без ограничения пикер показывает
// вообще все файлы — пользователь сам выбирает нужный, а какой это тип
// (видео/картинка) — определяем уже после выбора, см. isImageFile ниже.
videoBgFileInput.hidden = true;
document.body.appendChild(videoBgFileInput);

// Список расширений — ЗАПАСНОЙ путь для isImageFile ниже, на случай если
// file.type у выбранного файла пустой (случается на части
// Android-пикеров для файлов не из "родной" галереи, та же история уже
// была с аудио) — тогда ориентируемся на расширение имени файла.
...
/** Строго PNG (не любая картинка) — см. bgOverlayEl ниже: только PNG
 * трактуется как декоративный оверлей с прозрачностью, а не как замена
 * фона. Та же логика запасного пути по расширению, что и в isImageFile. */
...
let videoBgEl = null;
// Кастомная ФОТО-подложка (см. isImageFile выше) — та же роль, что и у
// videoBgEl, только <img>, не <video> (статичная картинка, ни play/pause,
// ни autoplay/loop ей не нужны). Отдельная переменная, а не попытка
// использовать videoBgEl для обоих случаев — типы элементов разные
// (HTMLImageElement/HTMLVideoElement), проще не смешивать.
let bgImageEl = null;
// ВРЕМЕННО (по просьбе пользователя — "загрузка PNG должна ложиться
// ПОВЕРХ текущего фона/визуализатора, не заменять его") — ОТДЕЛЬНЫЙ слой
// от bgImageEl/videoBgEl выше. Ключевые отличия:
//  - НЕ прячет дефолтное видео/звёздный туннель/волну/уже загруженный
//    фон — существует параллельно с ними, а не вместо;
//  - z-index между визуализатором (0, см. soundWave.js/starTunnel.js) и
//    3D-сценой (2, см. scene.js) — то есть ложится НАД волной/туннелем/
//    видео-фоном, но ПОД самим персонажем. Совпадает по слою с
//    edgeBlurCanvas (см. main.js) — тот почти всегда выключен
//    (временная фича), реальный конфликт маловероятен, но если когда-
//    нибудь понадобится включить оба разом, придётся развести z-index
//    явно (та же оговорка уже была для убранного слоя лучей у шара).
let bgOverlayEl = null;
videoBgFileInput.addEventListener("change", () => {
  const file = videoBgFileInput.files?.[0];
  if (!file) return;

  // PNG — отдельная ветка, полностью в сторону от остальной логики ниже
  // (не трогает videoBgEl/bgImageEl/дефолтное видео/туннель никак).
  if (isPngFile(file)) {
    if (bgOverlayEl) {
      bgOverlayEl.remove();
      URL.revokeObjectURL(bgOverlayEl.src);
      bgOverlayEl = null;
    }
    bgOverlayEl = document.createElement("img");
    bgOverlayEl.src = URL.createObjectURL(file);
    bgOverlayEl.style.position = "absolute";
    bgOverlayEl.style.top = "-1px";
    bgOverlayEl.style.left = "-1px";
    bgOverlayEl.style.width = "calc(100% + 2px)";
    bgOverlayEl.style.height = "calc(100% + 2px)";
    bgOverlayEl.style.objectFit = "cover";
    bgOverlayEl.style.zIndex = "1"; // между волной/туннелем (0) и 3D-сценой (2) — см. комментарий у объявления выше
    bgOverlayEl.style.pointerEvents = "none";
    bgOverlayEl.style.borderRadius = "inherit";
    container.appendChild(bgOverlayEl);
    bgOverlayEl.style.display = "block";
    videoBgFileInput.value = "";
    return;
  }

  // Любой НЕ-PNG фон/видео — по прямой просьбе пользователя убирает
  // активный PNG-оверлей (если был), не оставляя его "висеть" поверх
  // нового, не связанного с ним фона.
  if (bgOverlayEl) {
    bgOverlayEl.remove();
    URL.revokeObjectURL(bgOverlayEl.src);
    bgOverlayEl = null;
  ...
  if (videoBgEl) {
    videoBgEl.remove();
    URL.revokeObjectURL(videoBgEl.src);
    videoBgEl = null;
  }
  if (bgImageEl) {
    bgImageEl.remove();
  ...
  // Прячем дефолтное циклическое видео и звёздный туннель в любом
  // случае — та же логика "явно выбрал — сразу вижу", что была раньше
  // только для видео, теперь общая для обоих типов файлов.
  defaultVideoBgEl.style.display = "none";
  defaultVideoBgEl.pause();
  starTunnel.setVisible(false);

  if (isImageFile(file)) {
    bgImageEl = document.createElement("img");
    bgImageEl.src = URL.createObjectURL(file);
    bgImageEl.style.position = "absolute";
    // Тот же запас в 1px по периметру, что и у videoBgEl ниже (см.
    // подробный комментарий там про object-fit:cover и округление) —
    // одинаковая механика масштабирования что для видео, что для фото,
    // логично держать одинаковый запас.
    bgImageEl.style.top = "-1px";
    bgImageEl.style.left = "-1px";
    bgImageEl.style.width = "calc(100% + 2px)";
    bgImageEl.style.height = "calc(100% + 2px)";
    bgImageEl.style.objectFit = "cover";
    bgImageEl.style.zIndex = "-1";
    bgImageEl.style.borderRadius = "inherit"; // те же скруглённые углы, что и у самой сцены
    container.appendChild(bgImageEl);
    bgImageEl.style.display = "block"; // явно показываем — та же логика "явно выбрал — сразу вижу", что и у видео ниже
    videoBgFileInput.value = "";
    return;
  }

  videoBgEl = document.createElement("video");
  videoBgEl.src = URL.createObjectURL(file);
  videoBgEl.autoplay = true;
  videoBgEl.loop = true;
  videoBgEl.muted = true; // без этого автоплей может быть заблокирован браузером даже по пользовательскому жесту
  videoBgEl.playsInline = true; // без этого на iOS видео норовит открыться на весь экран вместо проигрывания внутри страницы
  videoBgEl.style.position = "absolute";
  // На 1px БОЛЬШЕ рамки со всех сторон (не ровно 0/100%) — гарантированный
  // запас на случай субпиксельной ошибки округления у object-fit:cover
  // (масштаб видео зависит от соотношения сторон видео/контейнера,
  // округление может "не дотянуть" на долю пикселя на некоторых
  // значениях — например, замечено на масштабе сцены 70%, полоска
  // красного диагностического слоя снизу). #scene-container уже имеет
  // overflow:hidden — лишний пиксель по периметру просто обрежется, не
  // нужно точно диагностировать саму ошибку округления, достаточно
  // перекрыть её запасом.
  videoBgEl.style.top = "-1px";
  videoBgEl.style.left = "-1px";
  videoBgEl.style.width = "calc(100% + 2px)";
  videoBgEl.style.height = "calc(100% + 2px)";
  videoBgEl.style.objectFit = "cover";
  videoBgEl.style.zIndex = "-1";
  videoBgEl.style.borderRadius = "inherit"; // те же скруглённые углы, что и у самой сцены
  container.appendChild(videoBgEl);
  videoBgEl.style.display = "block"; // явно показываем, поверх текущего состояния цикла кнопки волны
  videoBgFileInput.value = "";
});
...
(function setupVideoBgToggle() {
  ...
  btn.className = "mobile-fade-btn";
  btn.textContent = "🎬";
  btn.title = "ВРЕМЕННО: загрузить видео/фото как фон, или PNG как оверлей поверх (тест)";
  btn.style.position = "fixed";
  btn.style.top = "64px";
  btn.style.left = "12px"; // под кнопкой Settings (та тоже left:12px, top:12px)
  btn.style.zIndex = "9999";
  btn.style.width = "44px";
  btn.style.height = "44px";
  btn.style.border = "2px solid rgba(255,255,255,0.6)";
  ...
  btn.addEventListener("click", () => videoBgFileInput.click());
  document.body.appendChild(btn);
  mobileFadeButtons.push(btn);
})();

// ВРЕМЕННО (тест) — переключатель дискотечных прожекторов (см.
// discoLights.js). Независимо от цикла кнопки волны (фон и освещение
// персонажа — разные вещи, могут работать вместе с любым фоном).
(function setupDiscoLightsToggle() {
  const btn = document.createElement("button");
  btn.className = "mobile-fade-btn";
  btn.textContent = "🪩";
  btn.title = "ВРЕМЕННО: дискотечные прожекторы (тест)";
  btn.style.position = "fixed";
  btn.style.top = "116px"; // под кнопкой видео (64px) — та под Settings (12px)
  btn.style.left = "12px";
  btn.style.zIndex = "9999";
  btn.style.width = "44px";
  btn.style.height = "44px";
  ...
  btn.style.justifyContent = "center";
  btn.style.fontSize = "18px";
  // Три состояния по кругу (не просто вкл/выкл): свет+хелпер → свет без
  // хелпера → выключено → снова свет+хелпер...
  const DISCO_STATES = ["lights-and-helper", "lights-only", "off"];
  let discoStateIndex = 2; // стартуем с "off" — первый клик сразу даёт "lights-and-helper"
  btn.addEventListener("click", () => {
    discoStateIndex = (discoStateIndex + 1) % DISCO_STATES.length;
    const state = DISCO_STATES[discoStateIndex];
    if (state === "off") {
      discoLights.setEnabled(false);
    } else {
      discoLights.setEnabled(true);
      discoLights.setHelpersVisible(state === "lights-and-helper");
    }
    btn.style.background = state === "off" ? "rgba(0,0,0,0.4)" : "rgba(184,59,255,0.55)"; // лёгкая подсветка кнопки, когда свет включён (в любом из двух "включённых" состояний) — видно текущее состояние без дополнительного текста
  });
  document.body.appendChild(btn);
  mobileFadeButtons.push(btn);
})();

// ВРЕМЕННО (был A/B-тест, сейчас завершён) — источник пульсации шара.
// Перепробовали три варианта: "bass" (убран раньше, остался только для
// света/визуализатора), "flux" (spectral flux) и "steady" (метроном по
// оценке темпа, см. createSteadyTempoTracker) — по итогам живого теста
// пользователь выбрал "steady" как явно лучший вариант. Кнопка-
// переключатель (была тут) УБРАНА — с одним оставшимся вариантом
// переключать уже нечего. Сам алгоритм flux (fluxBeatDetector,
// analyzer.getSpectralFlux, вся логика ниже) НЕ УДАЛЁН по прямой
// просьбе пользователя — может пригодиться в будущем для чего-то
// другого — просто ПЕРЕСТАЛ СЧИТАТЬСЯ каждый кадр (см.
// BALL_PULSE_FLUX_ENABLED ниже), чтобы не тратить ресурсы на
// невостребованный сейчас расчёт.
const ballPulseSource = "steady";

// ВРЕМЕННО (по просьбе пользователя — "видно было на что именно
// реагирует шар") — плавающая текстовая панель-лог. На каждый удар,
// который реально доходит до пульсации шара (ballBeat, см. render-loop
// ниже), сюда дописывается строка: время, какой источник сейчас выбран
// (bass/flux), и числовое значение strength (во сколько раз сигнал был
// выше адаптивного порога детектора в момент срабатывания) — так видно
// не только САМ факт срабатывания, но и НАСКОЛЬКО уверенно сработал
// детектор, если вдруг покажется, что реагирует на явный шум (strength
// близко к порогу — подозрительно, сильно выше — скорее настоящий удар).
//
// ОТКЛЮЧЕНО (флаг, не удаление) — тест закончен, панель и сама сборка
// строки лога (см. вызов ниже, в render loop) больше не нужны, но код
// оставлен на будущее — вдруг снова понадобится смотреть, на что
// реагирует шар. При false ниже панель даже не создаётся в DOM (не
// просто спрятана через CSS) — ноль лишних элементов и ноль работы по
// сборке строки лога каждый удар.
const BALL_PULSE_LOG_ENABLED = false;
let ballPulseLogEl = null;
const ballPulseLogLines = [];
const BALL_PULSE_LOG_MAX_LINES = 20;
if (BALL_PULSE_LOG_ENABLED) {
  (function setupBallPulseLogPanel() {
    ...
    el.style.maxHeight = "70vh"; // подобрано под 20 строк при текущем font/line-height (10px/1.4)
    el.style.overflowY = "auto";
    el.style.background = "rgba(0,0,0,0.65)";
    el.style.color = "#0f0";
    el.style.font = "10px/1.4 monospace";
    el.style.padding = "6px 8px";
    el.style.borderRadius = "6px";
    el.style.whiteSpace = "pre";
    el.style.pointerEvents = "none"; // не мешает тапам под собой
...
/**
 * Дописывает строку в ballPulseLogPanel — источник + ВСЕ составляющие
 * формулы strength (см. beatDetector.js): strength = smoothed/threshold,
 * threshold = runningAverage × sensitivity. raw — сигнал ДО сглаживания
 * внутри детектора (для flux — это то, что вернул getSpectralFlux() в
 * этом кадре; для bass — тот же features.bass).
 */
function logBallPulse(source, detailStr) {
  if (!ballPulseLogEl) return;
  const now = new Date();
  const timeStr = now.toLocaleTimeString("ru-RU", { hour12: false }) + "." + String(now.getMilliseconds()).padStart(3, "0");
  ballPulseLogLines.push(`${timeStr} ${source} ${detailStr}`);
  if (ballPulseLogLines.length > BALL_PULSE_LOG_MAX_LINES) ballPulseLogLines.shift();
  ballPulseLogEl.textContent = ballPulseLogLines.join("\n");
}

// --- Кнопка смены режимов Sound Wave (справа от кнопки камеры) ---
// ВРЕМЕННО расширено до 13 режимов по кругу (было 3) — "blur" (размытие,
// по умолчанию) → "normal" (чёткие столбики) → "off" (ничего не видно,
// виден собственный фон #scene-container, #0d0d0d — см. style.css) →
// "startunnel" (звёздный туннель, см. starTunnel.js) → 9 дефолтных видео
// по очереди → снова "blur"... Значения ниже — финальные, подобраны и
// присланы пользователем через временную панель настройки (см. её код
// чуть выше, сейчас спрятана display:none, не удалена).
const WAVE_MODE_ORDER = [
  "blur",
  "normal",
  "off",
  "startunnel",
  ...DEFAULT_BG_VIDEOS.map((_, i) => `video_${i}`),
];
const MODE_SETTINGS = {
  ...
  // "off"/"video_N" ничего из этого не используют — просто прячут волну целиком.
};
...
function applyWaveMode(mode) {
  // Сброс — каждый режим явно решает, что показать, остальное прячем.
  // Дефолтное фоновое видео, кастомные загруженные видео/фото (кнопка 🎬)
  // и звёздный туннель — все взаимоисключающие с волной и друг с другом
  // (одинаковый z-index, иначе накладывались бы друг на друга) — проще
  // один раз спрятать все четыре, чем в каждой ветке помнить про них.
  if (defaultVideoBgEl.style.display !== "none") {
    defaultVideoBgEl.style.display = "none";
    defaultVideoBgEl.pause(); // не грузить/декодировать кадры видео, которое сейчас не видно
  }
  if (videoBgEl) videoBgEl.style.display = "none";
  if (bgImageEl) bgImageEl.style.display = "none";
  starTunnel.setVisible(false);

  if (mode === "blur" || mode === "normal") {
  ...
  soundWave.setVisible(false); // и "off", и "startunnel", и "video_N" — обычная волна не видна

  if (mode === "off") {
    // Ничего дополнительно показывать не нужно — собственный фон
    // #scene-container (#0d0d0d, см. style.css) и так уже под всеми
    // слоями, просвечивает сам.
    return;
  }

  if (mode === "startunnel") {
    starTunnel.setVisible(true);
    return;
  }

  // "video_N" — достаём индекс из имени режима, подставляем нужный файл.
  const videoIndex = Number(mode.slice("video_".length));
  const src = DEFAULT_BG_VIDEOS[videoIndex];
  // new URL(..., location.href).href — абсолютный путь, чтобы сравнение
  // ниже (защита от лишней перезагрузки того же файла) точно совпадало с
  // тем, что браузер уже хранит в defaultVideoBgEl.src (тот всегда
  // абсолютный, даже если присвоили относительный).
  const absoluteSrc = new URL(src, location.href).href;
  if (defaultVideoBgEl.src !== absoluteSrc) {
    defaultVideoBgEl.src = src;
  }
  defaultVideoBgEl.style.display = "block";
  defaultVideoBgEl.play().catch(() => {}); // на всякий случай, если автоплей не подхватился сам после смены display
}
...
// Название трека под сценой (именно этот элемент, не название в списке
// плейлиста и не имя файла) — максимум 25 символов, включая пробелы,
// остальное заменяется многоточием. Не через CSS text-overflow:ellipsis
// (та обрезает по ПИКСЕЛЬНОЙ ширине, зависит от размера экрана/шрифта) —
// здесь просили именно фиксированное количество СИМВОЛОВ.
const TRACK_TITLE_MAX_CHARS = 25;
function truncateTrackTitle(text) {
  if (text.length <= TRACK_TITLE_MAX_CHARS) return text;
  return text.slice(0, TRACK_TITLE_MAX_CHARS) + "...";
}
...
const playlistSheetHandle = document.getElementById("playlist-sheet-handle"); // хендл внутри самой шторки — виден, когда старый (в #player) скрыт под ней
...
// Сворачивание/разворачивание всей debug-панели целиком (см. index.html
// #audio-debug-toggle) — панель тестовая, постоянно занимает место по
// высоте, поджимая сцену; нужно уметь временно убрать её с экрана, не
// удаляя саму фичу.
const audioDebugToggleBtn = document.getElementById("audio-debug-toggle");
const audioDebugBarsEl = document.getElementById("audio-debug-bars");
audioDebugToggleBtn.addEventListener("click", () => {
  const isExpanded = audioDebugToggleBtn.getAttribute("aria-expanded") === "true";
  audioDebugToggleBtn.setAttribute("aria-expanded", String(!isExpanded));
  audioDebugBarsEl.classList.toggle("is-collapsed", isExpanded);
});

// ВРЕМЕННО (флаг, не удаление, по просьбе пользователя) — панель с
// параметрами (volume/bass/mid/... полосы) сейчас не нужна. При false
// ниже: сам заголовок-кнопка "▾ audio debug" скрывается совсем (не
// просто сворачиваются полосы — раньше даже в свёрнутом виде заголовок
// оставался виден и кликабелен), а полосы принудительно свёрнуты. Сам
// контейнер #audio-debug (см. index.html/style.css) НЕ трогаем — по
// словам пользователя, он, вероятно, нужен для разметки страницы
// (отступ между названием трека и панелью персонажей, см. комментарий в
// style.css у #audio-debug) — трогать вёрстку страницы для этого не
// стали, только спрятали содержимое. Второй эффект флага — см. вызов
// updateDebugBars() в render loop ниже: при выключенном флаге он вообще
// не вызывается каждый кадр, не тратит ресурсы на обновление невидимых
// полос.
const AUDIO_DEBUG_PANEL_ENABLED = false;
if (!AUDIO_DEBUG_PANEL_ENABLED) {
  audioDebugToggleBtn.style.display = "none";
  audioDebugToggleBtn.setAttribute("aria-expanded", "false");
  audioDebugBarsEl.classList.add("is-collapsed");
}

const dbgBarVolume = document.getElementById("dbg-bar-volume");
const dbgBarBass = document.getElementById("dbg-bar-bass");
const dbgBarMid = document.getElementById("dbg-bar-mid");
const dbgBarTreble = document.getElementById("dbg-bar-treble");
const dbgBarIntensity = document.getElementById("dbg-bar-intensity");
const dbgBarRiseRate = document.getElementById("dbg-bar-riserate");
const dbgBarBeat = document.getElementById("dbg-bar-beat");
const dbgBarStrongBeat = document.getElementById("dbg-bar-strongbeat");

/** Обновляет разом все полосы debug-панели (см. index.html #audio-debug).
 * riseRate нормализуется тем же коэффициентом (0.6), что и в
 * starTunnel.js — чтобы полоса визуально согласовывалась с тем, что
 * реально видно в самом визуализаторе, не жила отдельной шкалой. Без
 * аргументов — все полосы обнуляются (пауза/конец трека). */
function updateDebugBars({
  ...
  dbgBarVolume.style.width = `${Math.min(100, volume * 100)}%`;
  dbgBarBass.style.width = `${Math.min(100, bass * 100)}%`;
  dbgBarMid.style.width = `${Math.min(100, mid * 100)}%`;
  dbgBarTreble.style.width = `${Math.min(100, treble * 100)}%`;
  dbgBarIntensity.style.width = `${Math.min(100, intensity * 100)}%`;
  dbgBarRiseRate.style.width = `${Math.min(100, riseRate * 0.6 * 100)}%`;
  dbgBarBeat.style.width = `${Math.min(100, beatFlash * 100)}%`;
  dbgBarStrongBeat.style.width = `${Math.min(100, strongBeatFlash * 100)}%`;
}
...
// ВРЕМЕННО (флаг, не удаление) — по итогам A/B-теста пользователь выбрал
// steady-tempo вместо flux для пульсации шара (см. ballPulseSource
// ниже) — весь алгоритм flux (детектор + расчёт каждый кадр) оставлен в
// коде на будущее, но реально СЧИТАТЬСЯ перестал: false здесь отключает
// и сам вызов analyzer.getSpectralFlux(), и fluxBeatDetector.update() в
// render loop (см. блок анализа кадра ниже) — то есть не просто "не
// используется", а физически не выполняется, не тратит ресурсы кадра.
// Поставить true — снова включает всё как было, без других правок.
const BALL_PULSE_FLUX_ENABLED = false;
// ВРЕМЕННО (A/B-тест "варианта 1" из обсуждения с пользователем) —
// ВТОРОЙ, полностью независимый экземпляр того же самого детектора,
// только вместо features.bass ему каждый кадр скармливается
// analyzer.getSpectralFlux() (см. audioAnalyzer.js). createBeatDetector
// не завязан на bass жёстко — принимает любое 0..1 число, поэтому
// заводить отдельный детектор для другого сигнала можно один в один,
// без переписывания beatDetector.js. minBass ниже дефолтного (0.05) —
// сырой spectral flux обычно заметно мельче по абсолютной величине, чем
// bass (это разность соседних кадров, а не сам уровень), поэтому
// дефолтный порог тишины скорее всего резал бы вообще все события;
// точный порог не подбирался вживую — подкрутили вместе с пользователем
// по итогам A/B-теста (см. историю правок ниже); кнопка-переключатель,
// через которую тестировали, с тех пор убрана (см. BALL_PULSE_FLUX_ENABLED
// выше — flux проиграл steady-tempo).
// minBass ниже дефолтного — почти сырой spectral flux, оказалось,
// пропускал вообще любой мелкий шум спектра как "удар" (жаловались:
// "реагирует на всё подряд, как будто две пульсации вместо одной").
// ПОДКРУЧЕНО ПОСЛЕ ПЕРВОГО ЖИВОГО ТЕСТА:
//  - minBass поднят (0.005 → 0.03) — отсекаем мелкий шум, который
//    раньше проходил как события;
//  - smoothingFactor поднят (дефолт 0.4 → 0.6) — сильнее сглаживаем сам
//    сигнал flux ДО детектора; одиночный удар барабана нередко даёт не
//    один чистый пик, а короткую "рябь" из 2-3 соседних кадров (атака +
//    небольшой довесок на следующем кадре) — при слабом сглаживании это
//    читалось детектором как два отдельных события подряд, отсюда и
//    ощущение "двойной пульсации" на одном ударе;
//  - sensitivity чуть строже (дефолт 1.2 → 1.35) и cooldownMs длиннее
//    (дефолт 200 → 260) — дополнительная защита от повторного
//    срабатывания на хвосте того же самого удара.
// Цифры подобраны на глаз, не откалиброваны по записям — это отправная
// точка для дальнейшей подстройки по факту следующего теста.
const fluxBeatDetector = createBeatDetector({
  // Ужесточено ещё раз — по просьбе пользователя "реагировать ещё
  // меньше на лишний шум" (было: minBass 0.03, sensitivity 1.35,
  // smoothingFactor 0.6, cooldownMs 260):
  //  - minBass поднят ещё (0.03 → 0.05) — отсекаем больше мелкого шума
  //    как "недостаточно громкое, чтобы вообще считаться событием";
  //  - sensitivity строже (1.35 → 1.55) — событие должно заметнее
  //    выделяться на фоне недавнего среднего уровня flux, не просто
  //    чуть-чуть выше;
  //  - smoothingFactor ещё сильнее (0.6 → 0.75) — сигнал перед
  //    детектором сглаживается плотнее, мелкая рябь между кадрами не
  //    доходит до порога вообще;
  //  - cooldownMs длиннее (260 → 320) — дольше не даём повторно
  //    сработать сразу после предыдущего события.
  minBass: 0.05,
...
window.jerryFluxBeat = fluxBeatDetector; // debug: jerryFluxBeat.setSensitivity(1.1) — для тюнинга spectral flux независимо от bass-детектора

// ВРЕМЕННО (третий вариант A/B-теста пульсации шара, см. обсуждение с
// пользователем — "настукивание пальцем по столу"). Принципиально ИНАЯ
// идея, чем bass/flux выше: те — детекторы ОТДЕЛЬНЫХ всплесков (каждое
// срабатывание решается независимо, "выше порога прямо сейчас или нет"),
// поэтому реальная музыка (гост-ноты, синкопы, согласные в вокале) даёт
// неровный интервал между срабатываниями — визуально "танцует невпопад",
// даже если общая ЧАСТОТА срабатываний нормальная.
//
// steady-tempo вместо этого — не детектор конкретных ударов, а
// "метроном": оценивает ОБЩИЙ ТЕМП по недавним интервалам между
// bass-ударами (используем уже существующий основной beatDetector —
// тот, что и так постоянно работает для света/анимаций персонажей,
// заводить для этого ничего нового не нужно) и дальше тикает СВОИМ
// РОВНЫМ счётчиком фазы с этим периодом. Отдельные реальные удары не
// триггерят пульс шара напрямую — только медленно, через медиану
// последних интервалов, корректируют скорость тиканья. Именно это и
// должно дать ровное "1 2 3, 1 2 3" вместо "1 2, 1 2 3, 1...2 3".
function createSteadyTempoTracker({
  historySize = 8, // сколько последних интервалов между bass-ударами держим для оценки темпа
  minPeriodSec = 0.25, // 240 BPM — защита от патологически быстрой оценки (например, дребезг детектора)
  maxPeriodSec = 1.2, // 50 BPM — защита от патологически медленной оценки (пропуск нескольких тактов подряд)
  periodEaseTauSec = 2.5, // насколько плавно текущий период "доезжает" до новой оценки — чтобы смена темпа внутри трека не дёргала пульс рывком
  stopIntensity = 0.05, // ниже этого intensity считаем "тихо" — фаза замирает, пульс останавливается (см. update ниже)
  defaultPeriodSec = 0.5, // стартовая оценка (120 BPM), пока данных недостаточно
  // ВРЕМЕННО (по просьбе пользователя — "на быстрых песнях хотелось бы
  // ещё быстрее") — на медленных/средних треках оценка темпа уже
  // работает хорошо (сам пользователь так и сказал), трогать её там не
  // нужно. Но на быстрых треках bass-детектор (тот, из которого берутся
  // интервалы для оценки, см. update ниже) скорее всего чаще
  // недо-ловит близко идущие удары, чем на медленных — оттого оценка
  // темпа занижается именно там, где удары идут часто. Вместо того
  // чтобы трогать сам bass-детектор (это затронуло бы вообще всё —
  // свет, анимации персонажей), ускоряем ТОЛЬКО итоговое тиканье
  // steady-пульса, и ТОЛЬКО когда оценённый темп уже и так выше первого
  // порога — то есть на медленных треках ускорение попросту не
  // включается, поведение там не меняется.
  //
  // НЕСКОЛЬКО СТУПЕНЕЙ (не одна граница) — по просьбе пользователя:
  // список должен идти по возрастанию bpm, применяется САМАЯ ВЫСОКАЯ
  // ступень, чей порог уже пройден (см. getFastTempoMultiplier ниже) —
  // то есть трек на 105 BPM получит ×1.1, трек на 115 BPM — уже ×1.3,
  // трек на 200 BPM — ×1.4 (последняя ступень действует и для всего,
  // что выше её порога, дальше числа не растут без явного добавления
  // ещё одной ступени).
  fastTempoThresholds = [
    { bpm: 100, multiplier: 1.1 },
    { bpm: 110, multiplier: 1.2 },
    { bpm: 120, multiplier: 1.3 },
  ],
} = {}) {
  let intervals = []; // секунды, последние интервалы между bass-ударами (только "разумные", см. update)
  let lastBeatTime = null; // performance.now() последнего bass-удара, для расчёта интервала
  let targetPeriodSec = defaultPeriodSec; // "сырая" оценка — медиана последних интервалов
  let currentPeriodSec = defaultPeriodSec; // сглаженная версия (БЕЗ ускорения) — из неё же считается targetPeriodSec на будущих кадрах
  let phase = 0; // 0..1 — сколько от текущего такта уже "проехали"

  /** Ступенчатый выбор множителя — см. подробности у fastTempoThresholds
   * выше. Список должен быть отсортирован по возрастанию bpm (текущий
   * дефолт уже такой) — идём по порядку и берём последний подошедший
   * порог, поэтому более высокие ступени естественно перекрывают более
   * низкие. Ниже самого первого порога — множитель 1 (без ускорения). */
  function getFastTempoMultiplier(bpm) {
    let multiplier = 1;
    for (const step of fastTempoThresholds) {
      if (bpm >= step.bpm) multiplier = step.multiplier;
    }
    return multiplier;
  }
   ...
   * @param {boolean} bassBeatDetected - сработал ли в этом кадре ОСНОВНОЙ (bass) beatDetector — используется ТОЛЬКО для оценки темпа, не как прямой триггер пульса
   * @param {number} intensity - 0..1 (см. createIntensityTracker в audioAnalyzer.js) — на затухании фаза замирает
   * @param {number} [nowMs] - performance.now(), по умолчанию берётся сам
   * @returns {{ tick: boolean, periodSec: number, confident: boolean }} tick — true в кадре, где фаза перешла через 1 ("удар" метронома); periodSec — ФАКТИЧЕСКИЙ период тиканья (с учётом ускорения на быстром темпе, см. fastTempoThresholds), для debug-лога; confident — накопилось ли уже достаточно реальных интервалов (см. MIN_CONFIDENT_INTERVALS ниже), чтобы доверять periodSec — иначе это всё ещё стартовый defaultPeriodSec (или недалеко от него), которым НЕ стоит подменять что-либо, зависящее от точности оценки темпа (например, скорость танца персонажей, см. main.js)
   */
  function update(delta, bassBeatDetected, intensity, nowMs = performance.now()) {
    if (bassBeatDetected) {
      if (lastBeatTime !== null) {
        const intervalSec = (nowMs - lastBeatTime) / 1000;
        // Патологические интервалы (дребезг вплотную или огромный пропуск
        // такта) не пускаем в оценку темпа — они тянут медиану в сторону,
        // а не отражают реальный темп трека.
        if (intervalSec >= minPeriodSec && intervalSec <= maxPeriodSec) {
          intervals.push(intervalSec);
          if (intervals.length > historySize) intervals.shift();
          const sorted = [...intervals].sort((a, b) => a - b);
          // Медиана — устойчивее среднего к редким выбросам (например,
          // пропуск одного такта дал бы интервал вдвое больше нормы и
          // перекосил бы обычное среднее заметно сильнее).
          targetPeriodSec = sorted[Math.floor(sorted.length / 2)];
        }
      }
      lastBeatTime = nowMs;
    }

    const periodEase = 1 - Math.exp(-delta / periodEaseTauSec);
    currentPeriodSec += (targetPeriodSec - currentPeriodSec) * periodEase;

    // Ускорение ТОЛЬКО для быстрого темпа — см. подробное объяснение у
    // fastTempoThresholds выше. currentPeriodSec (без ускорения)
    // остаётся "чистой" оценкой темпа и продолжает использоваться как
    // есть для дальнейшего сглаживания (periodEase выше) — ускорение
    // применяется только в момент использования, не накапливается кадр
    // за кадром.
    const currentBpm = 60 / currentPeriodSec;
    const effectivePeriodSec = currentPeriodSec / getFastTempoMultiplier(currentBpm);

    // Нужно хотя бы 3 реальных интервала (не 1) — с одним-двумя медиана
    // слишком легко может оказаться случайно короткой (например, из-за
    // раннего ложного/повторного срабатывания сразу после сброса) и
    // временно "соврать" в сторону быстрого темпа.
    const MIN_CONFIDENT_INTERVALS = 3;
    const confident = intervals.length >= MIN_CONFIDENT_INTERVALS;
    // Пока НЕ confident — тикаем по нейтральному дефолтному периоду
    // (defaultPeriodSec, 120 BPM), а не по ещё недостоверной оценке. По
    // просьбе пользователя это теперь касается ВСЕХ потребителей
    // трекера, включая сам шар (раньше только main.js отдельно проверял
    // confident для персонажей, а шар брал periodSec напрямую, без
    // проверки) — раз трекер теперь общий, проще держать эту защиту
    // прямо тут, внутри самого тиканья фазы, чем повторять проверку в
    // каждом месте, где используется результат.
    const tickingPeriodSec = confident ? effectivePeriodSec : defaultPeriodSec;

    if (intensity < stopIntensity) {
      // Тихо — просто не двигаем фазу дальше (пульс "замирает" на месте,
      // НЕ сбрасываем её в 0 — чтобы на возврате музыки не было рывка).
      return { tick: false, periodSec: effectivePeriodSec, confident };
    }

    phase += delta / tickingPeriodSec;
    if (phase >= 1) {
      phase -= 1; // остаток, не жёсткий сброс в 0 — чтобы не копилась ошибка фазы на длинной дистанции
      return { tick: true, periodSec: effectivePeriodSec, confident };
    }
    return { tick: false, periodSec: effectivePeriodSec, confident };
  }

  /** Сбрасывает всё состояние — обязательно вызывать на pause/новом треке, как и у beatDetector/fluxBeatDetector. */
  function reset() {
    intervals = [];
    lastBeatTime = null;
    targetPeriodSec = defaultPeriodSec;
    currentPeriodSec = defaultPeriodSec;
    phase = 0;
  }

  return { update, reset };
}
// historySize: 6 — по итогам обсуждения с пользователем: изначально
// заводили ВТОРОЙ отдельный трекер именно из-за разницы в historySize
// (шару было 8, персонажам захотели 6) — раз в итоге решили, что 6
// вполне устроит и шар тоже, отдельный трекер для персонажей больше не
// нужен, снова один общий экземпляр на обоих.
const steadyTempoTracker = createSteadyTempoTracker({ historySize: 6 });
window.jerrySteadyTempo = steadyTempoTracker; // debug

// ВРЕМЕННО (тест звёздного туннеля, см. starTunnel.js) — отдельный
// маленький трекер "скорости нарастания громкости" (не саму громкость, а
// НАСКОЛЬКО БЫСТРО она сейчас растёт — 0, когда громкость падает или
// стоит на месте, положительное число, когда растёт, тем больше, чем
// резче нарастание). НЕ трогает существующий intensityTracker/волну/
// персонажей — полностью отдельная, новая логика, только для нового
// визуализатора.
function createVolumeRiseTracker() {
  let smoothedVolume = 0;
  let riseRate = 0;
  function update(volume, delta) {
    const safeDelta = Math.max(delta, 0.001); // защита от деления на ноль/на подозрительно маленький delta
    const rawRise = Math.max(0, volume - smoothedVolume) / safeDelta;
    // Сглаживаем саму громкость перед тем, как считать от неё нарастание —
    // иначе шумные мгновенные скачки анализатора читались бы как
    // "нарастание" на каждом кадре, а не только на настоящих музыкальных
    // разгонах.
    smoothedVolume += (volume - smoothedVolume) * Math.min(1, safeDelta * 8);
    // И саму скорость нарастания тоже сглаживаем — иначе сама riseRate
    // дёргалась бы кадр от кадра.
    riseRate += (rawRise - riseRate) * Math.min(1, safeDelta * 4);
    return riseRate;
  }
  function reset() {
    smoothedVolume = 0;
...
const volumeRiseTracker = createVolumeRiseTracker();
...
const player = createAudioPlayer({
  ...
  onPause() {
    ...
    beatDetector.reset();
    fluxBeatDetector.reset(); // тот же сброс нужен и второму (flux) детектору
    steadyTempoTracker.reset(); // и третьему (steady-tempo) — иначе история интервалов между ударами тоже тянет оценку темпа мимо реальности
    intensityTracker.reset();
    volumeRiseTracker.reset();
    ...
    updateDebugBars(); // обнуляем все полосы на паузе
  },
  onEnded() {
    ...
    clearNowPlayingHighlights();
    updateDebugBars(); // обнуляем все полосы по завершении трека
  },
});
...
async function applyActiveCharacters() {
  ...
  discoLights.setDuetMode(isDuet); // угол конуса прожекторов разный для соло/дуэта — см. discoLights.js
  desired.forEach((id) => {
    const slot = slots[id];
...
function loadTrackAtIndex(index) {
  ...
  trackTitleTextEl.textContent = title ? truncateTrackTitle(title) : t("track");
  ...
  fluxBeatDetector.reset(); // тот же сброс нужен и второму (flux) детектору
  steadyTempoTracker.reset(); // и третьему (steady-tempo)
  intensityTracker.reset();
  volumeRiseTracker.reset();
...
function setSheetFraction(fraction, { animate = false } = {}) {
  ...
  // ВАРИАНТ 3 (по договорённости) — шторка теперь отдельный слой поверх
  // всего (position:fixed, см. style.css), едет через transform, а не
  // делит место через flex-basis с #mobile-top-area. Из-за этого ВСЯ
  // логика ниже (схлопывание панели танцев/названия, отступ под сценой)
  // больше не нужна — сцена/панели просто НЕ ТРОГАЮТСЯ, шторка едет
  // поверх, ничего под собой не сжимая. Сама сцена/персонаж пока не
  // адаптируются под открытие шторки — это отдельный, следующий шаг.
  playlistSheet.style.transition = animate ? "transform 0.25s ease" : "none";
  playlistSheet.style.transform = `translateY(${(1 - clamped) * 100}%)`;
}
...
// ВАЖНО (история): раньше здесь была заморозка дорогого ресайза буферов
// сцены/волны на время активного перетаскивания хендла
// (suspendResizeTracking/resumeResizeTracking, вызывались в
// pointerdown/pointermove/pointerup ниже) — придумано под непрерывный
// поток событий изменения размера, когда шторка делила flex-basis со
// сценой и постоянно её сжимала во время перетаскивания. Шторка теперь
// отдельный слой поверх всего (position:fixed) и вообще не меняет
// размер сцены — того потока событий больше нет, замораживать нечего.
// Сами функции (setResizeSuspended) остались в scene.js/soundWave.js —
// готовый инструмент на будущее, если понадобится для чего-то другого,
// просто сейчас их никто не вызывает.

// Обе полоски-хендла (старая — в #player, новая — внутри самой шторки,
// видна когда старая скрыта под шторкой) управляют одним и тем же —
// вешаем одни и те же обработчики на оба сразу, через e.currentTarget
// (не жёстко playlistDragHandle), чтобы pointer capture захватывался и
// отпускался именно на том элементе, где реально начался жест.
const sheetHandles = [playlistDragHandle, playlistSheetHandle];

sheetHandles.forEach((handle) => {
...
function renderLoop() {
  ...
  let beatThisFrame = false; // обычный (не только "сильный") удар — для лёгкого мерцания в starTunnel
  // ВРЕМЕННО (см. BALL_PULSE_FLUX_ENABLED выше) — те же самые события
  // (beat/strong), только посчитанные от analyzer.getSpectralFlux()
  // вместо features.bass. Сейчас не считаются (флаг выключен) и не
  // используются (ballPulseSource жёстко "steady") — переменные
  // оставлены объявленными для совместимости остального кода ниже.
  let fluxBeatThisFrame = false;
  let fluxStrongBeatThisFrame = false;
  // "Супер-сильный" удар для третьей, более глубокой ступени пульсации
  // шара — эта ступень убрана (см. discoLights.js), но расчёт для flux
  // оставлен как есть, по просьбе пользователя не трогать flux вообще.
  // Версия для bass убрана вместе с самим bass-вариантом ниже (см.
  // ballPulseSource) — bass больше не участвует в пульсации шара,
  // только в визуализаторе/свете/анимациях персонажей.
  let fluxSuperStrongThisFrame = false;
  // Для debug-лога пульсации шара (см. logBallPulse выше) — те же самые
  // числа strength, что уже и так вычисляются каждый кадр внутри блока
  // ниже, просто выносим наружу, чтобы прочитать их там, где решается,
  // вызывать ли logBallPulse (в самом низу render-loop).
  let fluxStrengthThisFrame = 0;
  // Остальные составляющие strength (см. formulaу в beatDetector.js:
  // strength = smoothedBass/threshold, threshold = runningAverage×sensitivity)
  // — для подробного debug-лога (см. logBallPulse), чтобы видеть не
  // только итоговое число, но и из чего оно сложилось.
  let rawFluxThisFrame = 0; // сырой spectral flux ДО сглаживания внутри детектора
  let fluxSmoothedThisFrame = 0;
  let fluxRunningAverageThisFrame = 0;
  let fluxThresholdThisFrame = 0;
  // steady-tempo (третий вариант, см. createSteadyTempoTracker выше) —
  // tick (булево "метроном тикнул в этом кадре") и текущий сглаженный
  // период (для debug-лога).
  let steadyBeatThisFrame = false;
  let steadyPeriodSecThisFrame = 0.5;
  let steadyConfidentThisFrame = false; // накопилось ли достаточно реальных интервалов, чтобы доверять periodSec — см. confident в createSteadyTempoTracker
  let riseRateThisFrame = 0; // скорость нарастания громкости — для starTunnel
  let intensityThisFrame = 0; // для debug-панели ниже (та же величина, что уже видят персонажи)
  let featuresThisFrame = null; // для starTunnel.update() ниже — те же фичи, что уже читает этот кадр для персонажей/эквалайзера, не считаем повторно
  ...
  if (analyzer && musicIsPlaying) {
    ...
    // ВРЕМЕННО (по просьбе пользователя — flux снят с шара, но код
    // сохранён на будущее, см. комментарий у BALL_PULSE_FLUX_ENABLED) —
    // весь расчёт flux теперь считается ТОЛЬКО если флаг включён;
    // сейчас он выключен по умолчанию, поэтому этот блок физически не
    // выполняется каждый кадр — ноль лишней нагрузки, пока это не
    // понадобится снова.
    if (BALL_PULSE_FLUX_ENABLED) {
      // Диапазон сужен ещё раз (5000 → 3500) — по той же просьбе "меньше
      // реагировать на лишний шум": верхние-средние частоты (тарелки,
      // шипение, часть вокальных шипящих "с/ш") добавляли срабатывания,
      // не связанные с самим ударом.
      const flux = analyzer.getSpectralFlux(3500);
      const {
        beat: fluxBeat,
        strong: fluxStrongBeat,
        strength: fluxStrength,
        smoothedBass: fluxSmoothed, // несмотря на название поля (общее для детектора) — тут это сглаженный FLUX, не бас
        runningAverage: fluxRunningAverage,
        threshold: fluxThreshold,
      } = fluxBeatDetector.update(flux);
      fluxBeatThisFrame = fluxBeat;
      fluxStrongBeatThisFrame = fluxStrongBeat;
      // ×1.4 — строже дефолтного strongBeatRatio (1.35) у самого
      // детектора: это отдельный, более редкий "особый случай" (см.
      // обсуждение с пользователем), а не тот же самый strong. Было ×2,
      // потом ×1.5 — оба раза оказалось слишком строго (событие
      // срабатывало слишком редко), понижали дальше. Гейтим через
      // fluxBeat (детектор реально засчитал удар в этом кадре, с учётом
      // cooldown) — иначе strength могла бы разово подскочить выше порога
      // и между настоящими ударами, из-за сглаживания.
      fluxSuperStrongThisFrame = fluxBeat && fluxStrength >= 1.4;
      fluxStrengthThisFrame = fluxStrength;
      rawFluxThisFrame = flux;
      fluxSmoothedThisFrame = fluxSmoothed;
      fluxRunningAverageThisFrame = fluxRunningAverage;
      fluxThresholdThisFrame = fluxThreshold;
    }
    const intensity = intensityTracker.update(features, delta);
    const riseRate = volumeRiseTracker.update(features.volume, delta);
    ...
    beatThisFrame = beat;
    riseRateThisFrame = riseRate;
    intensityThisFrame = intensity;
    featuresThisFrame = features;
    // steady-tempo (см. createSteadyTempoTracker выше) — кормим ТОЛЬКО
    // обычным (не strong) bass-ударом, он используется лишь для оценки
    // темпа, не как прямой триггер.
    const steadyResult = steadyTempoTracker.update(delta, beat, intensity);
    steadyBeatThisFrame = steadyResult.tick;
    steadyPeriodSecThisFrame = steadyResult.periodSec;
    steadyConfidentThisFrame = steadyResult.confident;
    ...
    // ВРЕМЕННО-неВременно (по просьбе пользователя — "добавить steady-
    // tempo персонажам, не заменяя текущую логику") — коэффициент темпа
    // ДОБАВЛЯЕТСЯ поверх уже существующей intensity-логики скорости
    // танца (см. animationController.js reactToAudio), не вместо неё.
    // Использует ТОТ ЖЕ САМЫЙ трекер, что и шар (steadyTempoTracker,
    // historySize 6) — изначально заводили для этого отдельный
    // экземпляр (у шара тогда было 8, персонажам хотели 6), но по
    // итогам обсуждения решили, что 6 подходит и шару тоже — один общий
    // трекер снова, отдельный убрали.
    //
    // ОДНОСТОРОННЕ — только замедление, без ускорения: потолок 1 (не
    // 1.25, как было раньше) — по прямой просьбе пользователя: на
    // средних/быстрых треках персонажи и так уже хорошо двигались,
    // ускорять сверху не нужно, только придерживать на медляках. Нижняя
    // граница чуть снижена (0.85 → 0.8) — "ещё немного медленнее" по
    // той же просьбе.
    //
    // steadyConfidentThisFrame — пока трекер не набрал достаточно
    // реальных интервалов (см. confident в createSteadyTempoTracker),
    // periodSec ещё близок к стартовому "нейтральному" значению и может
    // быть недостоверным — на медленных треках это давало эффект
    // "танцует быстрее первые 10-15 секунд, потом наконец
    // замедляется" (см. обсуждение с пользователем). Пока !confident —
    // держим tempoFactor=1 (нейтрально), не подмешивая ещё недостоверную
    // оценку.
    const DANCE_TEMPO_NEUTRAL_BPM = 120;
    const bpm = 60 / steadyPeriodSecThisFrame;
    const tempoFactor = steadyConfidentThisFrame ? Math.min(1, Math.max(0.8, bpm / DANCE_TEMPO_NEUTRAL_BPM)) : 1;
    ...
    // ВРЕМЕННО (см. AUDIO_DEBUG_PANEL_ENABLED выше) — панель скрыта,
    // поэтому и сам вызов пропускаем целиком — незачем каждый кадр
    // собирать объект аргументов и писать 8 инлайн-стилей в невидимые
    // элементы.
    if (AUDIO_DEBUG_PANEL_ENABLED) {
      ...
      updateDebugBars({
        ...
        intensity,
        riseRate,
        // Линейное затухание от момента последнего удара — те же окна
        // (120мс/200мс), что раньше использовались как жёсткий порог
        // "включено/выключено" для точки-индикатора, теперь дают плавно
        // гаснущую полосу вместо резкого мигания.
        beatFlash: Math.max(0, 1 - (now - lastBeatFlashAt) / 120),
        strongBeatFlash: Math.max(0, 1 - (now - lastStrongBeatFlashAt) / 200),
      });
    }
  ...
  starTunnel.update(delta, featuresThisFrame, strongBeatThisFrame, beatThisFrame, riseRateThisFrame);
  // ВРЕМЕННО (был A/B-тест, сейчас завершён, см. ballPulseSource выше) —
  // discoLights.js сам по себе не знает, откуда взялось событие,
  // получает его уже готовым.
  const ballBeat = ballPulseSource === "flux" ? fluxBeatThisFrame : steadyBeatThisFrame;
  // ВРЕМЕННО (debug-лог, см. logBallPulse/BALL_PULSE_LOG_ENABLED выше) —
  // сборка строки лога (шаблонные строки + toFixed) — тоже не бесплатная
  // работа, поэтому пропускаем её целиком, если лог выключен, а не
  // просто не показываем результат.
  if (ballBeat && BALL_PULSE_LOG_ENABLED) {
    const detailStr =
      ballPulseSource === "flux"
        ? `raw=${rawFluxThisFrame.toFixed(3)} smooth=${fluxSmoothedThisFrame.toFixed(3)} avg=${fluxRunningAverageThisFrame.toFixed(3)} thr=${fluxThresholdThisFrame.toFixed(3)} str=${fluxStrengthThisFrame.toFixed(2)}`
        : `period=${(steadyPeriodSecThisFrame * 1000).toFixed(0)}ms bpm=${(60 / steadyPeriodSecThisFrame).toFixed(1)}`;
    logBallPulse(ballPulseSource, detailStr);
  }
  // Третья, более глубокая ступень пульсации (85% на "супер-сильном"
  // ударе, ×1.4 от порога) была убрана по просьбе пользователя — сам
  // расчёт fluxSuperStrongThisFrame (см. выше; версия для bass удалена
  // вместе с самим bass-вариантом) ОСТАВЛЕН как есть (пригодится, если
  // понадобится вернуть), просто сюда, в discoLights.update(), больше
  // не передаётся.
  discoLights.update(delta, intensityThisFrame, strongBeatThisFrame, ballBeat);
