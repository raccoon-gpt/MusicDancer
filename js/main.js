...
const steadyTempoTracker = createSteadyTempoTracker();
...
function renderLoop() {
  ...
  if (analyzer && musicIsPlaying) {
    ...
    // тот же самый steadyTempoTracker, что уже считает период для
    // пульсации шара (см. steadyResult выше), просто ещё раз
    // используется здесь. 120 BPM — "нейтральный" темп (совпадает с
    // дефолтным периодом трекера 0.5с, пока данных недостаточно) — при
    // нём коэффициент равен 1, то есть ничего не меняет по сравнению с
    // тем, что было раньше. Диапазон 0.85..1.25 подобран на глаз, чтобы
    // не давать совсем крайних значений (персонаж не должен ни
    // "зависать", ни двигаться абсурдно быстро) — та же логика
    // ограничения, что уже применяется у самого трекера для periodSec.
    //
    // steadyConfidentThisFrame — ВАЖНО: пока трекер не набрал достаточно
    ...
    // замедляется" (см. обсуждение с пользователем): персонажи какое-то
    // время получали tempoFactor, посчитанный от ещё не устоявшейся
    // оценки. Пока !confident — держим tempoFactor=1 (нейтрально, как и
    // было до этой правки), не подмешивая ещё недостоверную оценку.
    const DANCE_TEMPO_NEUTRAL_BPM = 120;
    const bpm = 60 / steadyPeriodSecThisFrame;
    const tempoFactor = steadyConfidentThisFrame ? Math.min(1.25, Math.max(0.85, bpm / DANCE_TEMPO_NEUTRAL_BPM)) : 1;
