...
function renderLoop() {
  ...
  // beatDetector.js — этот порог, наоборот, заметно строже, ×1.5, поэтому и
  ...
  if (analyzer && musicIsPlaying) {
    ...
    // ×1.5 — строже дефолтного strongBeatRatio (1.35) у самого
    // детектора: это отдельный, более редкий "особый случай" (см.
    // обсуждение с пользователем), а не тот же самый strong. Было ×2 —
    // оказалось слишком строго (событие срабатывало слишком редко),
    // понизили. Гейтим через beat/fluxBeat (детектор реально засчитал
    // удар в этом кадре, с учётом cooldown) — иначе strength могла бы
    // разово подскочить выше порога и между настоящими ударами, из-за
    // сглаживания.
    // Порог понижен с ×2 до ×1.5 — по факту теста ×2 оказалось слишком
    // строго (событие срабатывало слишком редко).
    beatSuperStrongThisFrame = beat && beatStrength >= 1.5;
    fluxSuperStrongThisFrame = fluxBeat && fluxStrength >= 1.5;
  ...
  // ×1.5, см. выше), НЕ обычный strong/fluxStrong детектора (тот был на
