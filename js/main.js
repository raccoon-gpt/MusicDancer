...
function renderLoop() {
  ...
  if (analyzer && musicIsPlaying) {
    ...
    const { beat: fluxBeat, strong: fluxStrongBeat } = fluxBeatDetector.update(flux);
  ...
  // ВРЕМЕННО (по просьбе пользователя) — в режиме flux переход на 91%
  // отключён, чтобы посмотреть на чистую 100-95-100 пульсацию без
  // отвлекающей более глубокой ступени. false вместо
  // fluxStrongBeatThisFrame — событие всё ещё считается (см. выше), просто
  // сюда сознательно не долетает. Легко вернуть обратно (заменить false
  // на fluxStrongBeatThisFrame), когда понадобится.
  const ballStrongBeat = ballPulseSource === "flux" ? false : strongBeatThisFrame;
