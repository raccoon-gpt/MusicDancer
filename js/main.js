...
// ВРЕМЕННО (по просьбе пользователя — калибровка позиции/размера
// дискошара вручную) — плавающая панель с тремя ползунками (вверх/вниз,
// вперёд/назад, размер) + живые числовые значения на экране. Пока
// панель на странице — discoLights.js полностью игнорирует свою обычную
// автоматику (соло/дуэт-переход, пульсацию под удар, см.
// setManualBallOverride в discoLights.js) и жёстко ставит шар туда, куда
// сказали ползунки — так подобранные цифры не "плывут" под пульсацией,
// пользователь видит ровно то значение, которое потом сообщит обратно.
(function setupBallTransformPanel() {
  const panel = document.createElement("div");
  panel.style.position = "fixed";
  ...
  // Стартовые значения — примерно соло-положение шара (см. SOLO_BALL_Y/
  // BALL_BASE_RADIUS в discoLights.js), просто разумная отправная точка
  // для калибровки, не что-то принципиальное.
  const state = { y: 1.6, z: -1.3, scale: 1 };
  ...
  function row(labelText, key, min, max, step) {
    ...
    label.textContent = `${labelText}: ${state[key].toFixed(2)}`;
    ...
    input.addEventListener("input", () => {
      ...
      label.textContent = `${labelText}: ${state[key].toFixed(2)}`;
      discoLights.setManualBallOverride({ ...state });
    });
  ...
  title.textContent = "ВРЕМЕННО: калибровка шара";
  title.style.fontWeight = "bold";
  panel.appendChild(title);

  panel.appendChild(row("Y (вверх/вниз)", "y", 0, 4, 0.05));
  panel.appendChild(row("Z (вперёд/назад)", "z", -5, 2, 0.05));
  panel.appendChild(row("Размер", "scale", 0.3, 3, 0.05));

  document.body.appendChild(panel);
  discoLights.setManualBallOverride({ ...state }); // применяем сразу — иначе до первого движения ползунка шар остаётся на обычной автоматике, а панель уже показывает свои числа
})();
...
function createSteadyTempoTracker({
   ...
   * @returns {{ tick: boolean, periodSec: number }} tick — true в кадре, где фаза перешла через 1 ("удар" метронома); periodSec — ФАКТИЧЕСКИЙ период тиканья (с учётом ускорения на быстром темпе, см. fastTempoThresholds), для debug-лога
   */
