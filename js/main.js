...
// ВРЕМЕННО (тестирование): загрузка видео как фон сцены — посмотреть,
// как персонажи танцуют "на видеофоне". Порядок слоёв (z-index, снизу
// вверх): видео (-1) → звуковая волна (0, см. soundWave.js) → слой
// размытия контура персонажа (1, см. main.js) → сам персонаж (2, см.
// scene.js) — волна и персонаж остаются НАД видео, как и просили.
// Рендерер уже полностью прозрачный (alpha:true + setClearAlpha(0), см.
// scene.js) — специально ничего дополнительно включать не нужно, видео
// уже будет видно сквозь персонажа/сцену без правок в scene.js.
const videoBgFileInput = document.createElement("input");
videoBgFileInput.type = "file";
videoBgFileInput.accept = "video/*";
