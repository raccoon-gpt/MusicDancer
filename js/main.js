...
// ВРЕМЕННО (A/B-тест) — переключатель источника пульсации шара. Раньше
// тут был ещё и "bass" (сигнал от features.bass через основной
// beatDetector) — по прямой просьбе пользователя УБРАН из этого
// переключателя: bass остаётся общим детектором для света/анимаций
// персонажей/визуализатора, но для самого шара больше не предлагается —
// его заменил третий вариант, "steady" (см. createSteadyTempoTracker).
// Теперь тут ДВА варианта: "flux" (spectral flux через отдельный
// fluxBeatDetector, см. audioAnalyzer.js getSpectralFlux) и "steady"
// (не детектор отдельных ударов, а "метроном" — ровный счётчик фазы,
// скорость которого медленно подстраивается под оценённый темп трека,
// см. createSteadyTempoTracker выше — придуман под задачу "танцевать
// В ТАКТ", а не ловить каждый удар точно). Не трогает свет/анимацию
// персонажей — влияет ТОЛЬКО на то, какое булево событие уходит в
// discoLights.update() как триггер пульсации размера шара (см.
// discoLights.js — сам он не знает, откуда взялось событие).
let ballPulseSource = "flux";
(function setupBallPulseSourceToggle() {
  const btn = document.createElement("button");
  btn.className = "mobile-fade-btn";
  btn.textContent = "🥁";
  btn.title = "ВРЕМЕННО: источник пульсации шара — flux (тест)";
  btn.style.position = "fixed";
  btn.style.top = "168px"; // под кнопкой диско-прожекторов (116px)
  btn.style.left = "12px";
  btn.style.zIndex = "9999";
  btn.style.width = "44px";
  btn.style.height = "44px";
  ...
  btn.style.borderRadius = "8px";
  btn.style.cursor = "pointer";
  btn.style.background = "rgba(59,223,255,0.55)"; // старт на flux — та же подсветка, что и раньше у flux
  btn.style.color = "#fff";
  btn.style.display = "flex";
  btn.style.alignItems = "center";
  btn.style.justifyContent = "center";
  btn.style.fontSize = "18px";
  btn.addEventListener("click", () => {
    ballPulseSource = ballPulseSource === "flux" ? "steady" : "flux";
    btn.title = `ВРЕМЕННО: источник пульсации шара — ${ballPulseSource} (тест)`;
    // Та же подсветка фона, что и раньше — "включено что-то
    // нестандартное" виднее без лишнего текста на кнопке. У steady —
    // оранжевый, чтобы визуально отличался от flux (голубой).
    btn.style.background = ballPulseSource === "flux" ? "rgba(59,223,255,0.55)" : "rgba(255,149,59,0.55)";
  });
  document.body.appendChild(btn);
  mobileFadeButtons.push(btn);
})();
...
// точный порог не подбирался вживую — это как раз то, что можно
// подкрутить вместе с пользователем после A/B-теста через кнопку
// переключения (см. setupBallPulseSourceToggle ниже).
...
function renderLoop() {
  ...
  // ВРЕМЕННО (A/B-тест "варианта 1", см. fluxBeatDetector выше) — те же
  // самые события (beat/strong), только посчитанные от
  // analyzer.getSpectralFlux() вместо features.bass. Какая из двух пар
  // реально уходит в discoLights.update() ниже (как события,
  // управляющие ступенчатой пульсацией шара) — решает ballPulseSource
  // (переключается кнопкой, см. setupBallPulseSourceToggle).
  ...
  if (analyzer && musicIsPlaying) {
    ...
    // ВРЕМЕННО (A/B-тест) — spectral flux считается сразу после
    // getFeatures() (переиспользует freqData этого же кадра, см.
    // audioAnalyzer.js) и гонится через СВОЙ отдельный beat-детектор —
    // никак не влияет на основной beatDetector/strongBeat выше (те
    // по-прежнему работают от bass, как и раньше, для света/анимации
    // персонажей).
    ...
    // и между настоящими ударами, из-за сглаживания. Оставлено только
    // для flux (см. комментарий у fluxSuperStrongThisFrame выше) — по
    // просьбе пользователя flux не трогаем, даже неиспользуемый расчёт.
  ...
  // ВРЕМЕННО (A/B-тест) — ballPulseSource переключается кнопкой (см.
  // setupBallPulseSourceToggle). discoLights.js сам по себе не знает
  // про выбор источника — просто получает уже выбранное событие
  // (ballBeat) как есть. strongBeatThisFrame (3-й аргумент, для вспышки
  // прожекторов) ВСЕГДА от баса, независимо от этого переключателя —
  // тумблер влияет ТОЛЬКО на пульсацию размера шара.
  const ballBeat = ballPulseSource === "flux" ? fluxBeatThisFrame : steadyBeatThisFrame;
  // ВРЕМЕННО (debug-лог, см. logBallPulse выше) — пишем строку в лог
  // ровно в тот момент, когда событие реально долетает до шара (то есть
  // после выбора источника кнопкой, не раньше).
  if (ballBeat) {
    const detailStr =
