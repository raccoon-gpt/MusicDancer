...
function createSteadyTempoTracker({
  ...
  let currentPeriodSec = defaultPeriodSec; // сглаженная версия — реально используется для тиканья фазы
   ...
   * @returns {{ tick: boolean, periodSec: number }} tick — true в кадре, где фаза перешла через 1 ("удар" метронома); periodSec — текущий (сглаженный) период, для debug-лога
   */
  function update(delta, bassBeatDetected, intensity, nowMs = performance.now()) {
    ...
    if (intensity < stopIntensity) {
      ...
      return { tick: false, periodSec: currentPeriodSec };
    }

    phase += delta / currentPeriodSec;
    if (phase >= 1) {
      phase -= 1; // остаток, не жёсткий сброс в 0 — чтобы не копилась ошибка фазы на длинной дистанции
      return { tick: true, periodSec: currentPeriodSec };
    }
    return { tick: false, periodSec: currentPeriodSec };
  }
