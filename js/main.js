...
function createSteadyTempoTracker({
  ...
  // steady-пульса, и ТОЛЬКО когда оценённый темп уже и так выше
  // fastTempoBpmThreshold — то есть на медленных/средних треках
  // множитель попросту не включается, поведение там не меняется.
  fastTempoBpmThreshold = 110, // ниже этого BPM ускорение не применяется вообще
  fastTempoSpeedMultiplier = 1.5, // во сколько раз ускоряем тиканье НАД порогом (число подобрано на глаз, не откалибровано)
} = {}) {
   ...
   * @returns {{ tick: boolean, periodSec: number }} tick — true в кадре, где фаза перешла через 1 ("удар" метронома); periodSec — ФАКТИЧЕСКИЙ период тиканья (с учётом ускорения на быстром темпе, см. fastTempoBpmThreshold), для debug-лога
   */
  function update(delta, bassBeatDetected, intensity, nowMs = performance.now()) {
    ...
    // fastTempoBpmThreshold/fastTempoSpeedMultiplier выше. currentPeriodSec
    // (без ускорения) остаётся "чистой" оценкой темпа и продолжает
    // использоваться как есть для дальнейшего сглаживания (periodEase
    // выше) — ускорение применяется только в момент использования, не
    // накапливается кадр за кадром.
    const currentBpm = 60 / currentPeriodSec;
    const effectivePeriodSec = currentBpm > fastTempoBpmThreshold ? currentPeriodSec / fastTempoSpeedMultiplier : currentPeriodSec;
