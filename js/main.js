...
// БЕЗ accept — та же история, что уже была с аудио: accept="video/*"
// ненадёжно распознаётся системным пикером на части файлов (не все
// .mp4 с телефона показывались как доступные для выбора), несмотря на
// то что они прекрасно воспроизводятся. Без ограничения пикер
// показывает вообще все файлы — пользователь сам выбирает нужный.
...
videoBgFileInput.addEventListener("change", () => {
  ...
  // Показываем сразу, поверх текущего состояния цикла кнопки волны — та
  // же логика "явно выбрал — сразу вижу", что и раньше. Прячем
  // дефолтное видео, если было активно, чтобы не накладывались друг на
  // друга (одинаковый z-index).
...
(function setupVideoBgToggle() {
  ...
  btn.className = "mobile-fade-btn";
  btn.textContent = "🎬";
  btn.title = "ВРЕМЕННО: загрузить видео как фон сцены (тест)";
  btn.style.position = "fixed";
  btn.style.top = "64px";
...
function applyWaveMode(mode) {
  ...
  // Дефолтное фоновое видео, кастомное загруженное видео (кнопка 🎬) и
  // звёздный туннель — все взаимоисключающие с волной и друг с другом
  // (одинаковый z-index, иначе накладывались бы друг на друга) — проще
  // один раз спрятать все три, чем в каждой ветке помнить про них.
