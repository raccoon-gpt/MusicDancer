...
// Загрузка видео как фон сцены — посмотреть, как персонажи танцуют "на
// видеофоне". Рендерер уже полностью прозрачный (alpha:true +
// setClearAlpha(0), см. scene.js) — специально ничего дополнительно
// включать не нужно.
...
let videoBgEl = null;
let videoBgVisible = true; // управляется циклом кнопки волны — см. applyWaveMode ниже
videoBgFileInput.addEventListener("change", () => {
  ...
  videoBgEl.style.display = videoBgVisible ? "block" : "none"; // подхватывает текущее состояние цикла кнопки волны, а не всегда "видимо"
...
// ВРЕМЕННО расширено до ЧЕТЫРЁХ режимов по кругу (было три) — для теста
// видео-фона/диагностики многослойности (см. блок выше): "blur"
// (размытие, по умолчанию) → "normal" (чёткие столбики) → "off" (волна
// выключена, видно видео-фон, если загружено, либо красный диагностический
// слой) → "off_no_video" (видео тоже выключено, видно ТОЛЬКО красный слой)
// → снова "blur" (и волна, и видео возвращаются). Значения ниже —
...
// удалена).
const WAVE_MODE_ORDER = ["blur", "normal", "off", "off_no_video"];
const MODE_SETTINGS = {
  ...
  // "off"/"off_no_video" ничего из этого не используют — просто прячут волну целиком.
};
...
function applyWaveMode(mode) {
  if (mode === "off" || mode === "off_no_video") {
    soundWave.setVisible(false);
    // "off" — видео (если загружено) остаётся видно, просто волна прячется.
    // "off_no_video" — прячется и видео тоже, видно только красный слой.
    videoBgVisible = mode !== "off_no_video";
    if (videoBgEl) videoBgEl.style.display = videoBgVisible ? "block" : "none";
    return;
  }
  // blur/normal — волна снова видна, и видео (если было спрятано на
  // предыдущем круге через "off_no_video") тоже возвращается — цикл
  // замкнутый, полное исходное состояние восстанавливается целиком.
  videoBgVisible = true;
  if (videoBgEl) videoBgEl.style.display = "block";
