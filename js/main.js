...
(function setupDiscoLightsToggle() {
  ...
  btn.style.justifyContent = "center";
  btn.style.fontSize = "18px";
  let discoLightsEnabled = false;
  btn.addEventListener("click", () => {
    discoLightsEnabled = !discoLightsEnabled;
    discoLights.setEnabled(discoLightsEnabled);
    btn.style.background = discoLightsEnabled ? "rgba(184,59,255,0.55)" : "rgba(0,0,0,0.4)"; // лёгкая подсветка кнопки, когда включено — видно текущее состояние без дополнительного текста
  });
  document.body.appendChild(btn);
  mobileFadeButtons.push(btn);
})();
