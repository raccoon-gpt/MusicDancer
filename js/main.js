...
// ВРЕМЕННО (A/B-тест "варианта 1" — spectral flux вместо bass для
// пульсации дискошара, см. обсуждение выше и fluxBeatDetector) —
// переключатель источника пульсации. "bass" — как было раньше (сигнал
// от features.bass через основной beatDetector), "flux" — новый вариант
// (spectral flux через отдельный fluxBeatDetector, см. audioAnalyzer.js
// getSpectralFlux). Не трогает свет/анимацию персонажей — влияет ТОЛЬКО
// на то, какое число уходит в discoLights.update() как beatStrength для
// пульсации размера шара (см. discoLights.js — сам он не знает, откуда
// взялось число, разница только тут).
let ballPulseSource = "bass";
(function setupBallPulseSourceToggle() {
  const btn = document.createElement("button");
  btn.className = "mobile-fade-btn";
  btn.textContent = "🥁";
  btn.title = "ВРЕМЕННО: источник пульсации шара — bass (тест)";
  btn.style.position = "fixed";
  ...
  btn.style.fontSize = "18px";
  btn.addEventListener("click", () => {
    ballPulseSource = ballPulseSource === "bass" ? "flux" : "bass";
    btn.title = `ВРЕМЕННО: источник пульсации шара — ${ballPulseSource} (тест)`;
    // Та же подсветка фона, что и у кнопки диско-прожекторов — "включено
    // что-то нестандартное" виднее без лишнего текста на кнопке.
    btn.style.background = ballPulseSource === "flux" ? "rgba(59,223,255,0.55)" : "rgba(0,0,0,0.4)";
  });
  document.body.appendChild(btn);
  mobileFadeButtons.push(btn);
})();
 ...
 */
function logBallPulse(source, details) {
  if (!ballPulseLogEl) return;
  const now = new Date();
  const timeStr = now.toLocaleTimeString("ru-RU", { hour12: false }) + "." + String(now.getMilliseconds()).padStart(3, "0");
  const { raw, smoothed, runningAverage, threshold, strength } = details;
  ballPulseLogLines.push(
    `${timeStr} ${source} raw=${raw.toFixed(3)} smooth=${smoothed.toFixed(3)} avg=${runningAverage.toFixed(3)} thr=${threshold.toFixed(3)} str=${strength.toFixed(2)}`
  );
...
// cooldownMs поднят с дефолтных 200 до 280 — по логу debug-панели (см.
// logBallPulse) видно было срабатывания впритык к 200мс (например
// 208мс между соседними событиями) — заметно чаще, чем реальный бит
// хип-хопа (обычно 500-650мс между ударами), похоже на то, что между
// настоящими ударами баса детектор цеплял что-то ещё как отдельное
// событие. ВАЖНО: это ОБЩИЙ детектор — не только для пульсации шара в
// bass-режиме, но и для вспышек прожекторов, акцентных анимаций
// персонажей (reactToAudio) и мерцания в starTunnel — правка затрагивает
// всё это разом, не только шар.
const beatDetector = createBeatDetector({ cooldownMs: 280 }); // не зависит от analyzer — можно создать сразу
...
const player = createAudioPlayer({
  ...
  onPause() {
    ...
    beatDetector.reset();
    fluxBeatDetector.reset(); // тот же сброс нужен и второму (flux) детектору, иначе на паузе/новом треке его SMA-история тоже тянет baseline мимо реальности
...
function renderLoop() {
  ...
  // шара (85%, см. discoLights.js) — ОТДЕЛЬНО от strong/fluxStrong выше
  // (те срабатывают уже на ×1.35 от порога, см. strongBeatRatio в
  // beatDetector.js — этот порог, наоборот, заметно строже, ×1.4, поэтому и
  // считается прямо тут, а не переиспользует поле strong самого
  // детектора). У обоих источников (bass/flux) — своя версия, тумблер
  // ballPulseSource ниже выбирает нужную, как и с остальными парами.
  let beatSuperStrongThisFrame = false;
  ...
  let beatSmoothedThisFrame = 0;
  let beatRunningAverageThisFrame = 0;
  let beatThresholdThisFrame = 0;
  ...
  if (analyzer && musicIsPlaying) {
    ...
    const {
      ...
      smoothedBass: beatSmoothed,
      runningAverage: beatRunningAverage,
      threshold: beatThreshold,
    } = beatDetector.update(features.bass);
    ...
    // beat/fluxBeat (детектор реально засчитал удар в этом кадре, с
    // учётом cooldown) — иначе strength могла бы разово подскочить выше
    // порога и между настоящими ударами, из-за сглаживания.
    // Порог понижен: было ×2, потом ×1.5, сейчас ×1.4 — по фактам теста
    // предыдущие оказывались слишком строгими (событие срабатывало
    // слишком редко).
    beatSuperStrongThisFrame = beat && beatStrength >= 1.4;
    fluxSuperStrongThisFrame = fluxBeat && fluxStrength >= 1.4;
    beatStrengthThisFrame = beatStrength;
    fluxStrengthThisFrame = fluxStrength;
    rawFluxThisFrame = flux;
    beatSmoothedThisFrame = beatSmoothed;
    beatRunningAverageThisFrame = beatRunningAverage;
    beatThresholdThisFrame = beatThreshold;
  ...
  const ballBeat = ballPulseSource === "flux" ? fluxBeatThisFrame : beatThisFrame;
  ...
  if (ballBeat) {
    const details =
      ballPulseSource === "flux"
        ? {
            raw: rawFluxThisFrame,
            smoothed: fluxSmoothedThisFrame,
            runningAverage: fluxRunningAverageThisFrame,
            threshold: fluxThresholdThisFrame,
            strength: fluxStrengthThisFrame,
          }
        : {
            raw: featuresThisFrame ? featuresThisFrame.bass : 0,
            smoothed: beatSmoothedThisFrame,
            runningAverage: beatRunningAverageThisFrame,
            threshold: beatThresholdThisFrame,
            strength: beatStrengthThisFrame,
          };
    logBallPulse(ballPulseSource, details);
  }
  // Третья, более глубокая ступень пульсации (85% на "супер-сильном"
  // ударе, ×1.4 от порога) была убрана по просьбе пользователя — сам
  // расчёт beatSuperStrongThisFrame/fluxSuperStrongThisFrame (см. выше)
  // ОСТАВЛЕН как есть (пригодится, если понадобится вернуть), просто
  // сюда, в discoLights.update(), больше не передаётся.
