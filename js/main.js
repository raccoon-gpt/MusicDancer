...
function renderLoop() {
  ...
  if (analyzer && musicIsPlaying) {
    ...
    // персонажей).
    // Диапазон сужен с дефолтных 8000 до 5000 — после первого теста
    // решили, что верхи (тарелки/шипение) добавляли лишние срабатывания
    // поверх основного удара, усиливая ощущение "двойной пульсации".
    const flux = analyzer.getSpectralFlux(5000);
  ...
  // про выбор источника — просто получает уже выбранные события
  // (ballBeat/ballStrongBeat) как есть. strongBeatThisFrame (3-й
  // аргумент, для вспышки прожекторов) ВСЕГДА от баса, независимо от
  // этого переключателя — тумблер влияет ТОЛЬКО на ступенчатую
  // пульсацию размера шара.
  const ballBeat = ballPulseSource === "flux" ? fluxBeatThisFrame : beatThisFrame;
  // ballStrongBeat теперь — это beatSuperStrong/fluxSuperStrong (порог
  // ×1.4, см. выше), НЕ обычный strong/fluxStrong детектора (тот был на
  // ×1.35 — по просьбе пользователя убрали как триггер третьей ступени,
  // порог показался слишком мягким/частым). Название параметра в
  // discoLights.update() осталось прежним (ballStrongBeat) — он и
  // раньше был "событие для более глубокой ступени", просто теперь
  // порог этого события другой.
  const ballStrongBeat = ballPulseSource === "flux" ? fluxSuperStrongThisFrame : beatSuperStrongThisFrame;
  discoLights.update(delta, intensityThisFrame, strongBeatThisFrame, ballBeat, ballStrongBeat);
