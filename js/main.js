...
// ВРЕМЕННО расширено до 12 режимов по кругу (было 3) — "blur" (размытие,
// по умолчанию) → "normal" (чёткие столбики) → "off" (волна не видна,
// виден собственный фон #scene-container, #0d0d0d — см. style.css) → 9
// дефолтных видео по очереди (тоже волна не видна — по договорённости
// волна поверх видео больше не нужна, не понравилось как смотрится) →
// снова "blur"... Значения ниже — финальные, подобраны и присланы
// пользователем через временную панель настройки (см. её код чуть выше,
// сейчас спрятана display:none, не удалена).
...
function applyWaveMode(mode) {
  ...
  // Дефолтное фоновое видео и кастомное загруженное видео (кнопка 🎬) —
  // оба взаимоисключающие с волной и друг с другом (одинаковый
  // z-index, иначе накладывались бы друг на друга) — проще один раз
  // спрятать оба, чем в каждой ветке помнить про них.
  ...
  soundWave.setVisible(false); // и "off", и "video_N" — волна не видна
