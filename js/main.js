...
// ВРЕМЕННО (тестирование, диагностика многослойности): красный фон —
// самый нижний слой, ВСЕГДА присутствует, никогда не отключается ни
// кнопкой волны, ни чем-либо ещё — чтобы никогда не было "дыры" (видно
// сквозь всё до самой страницы), и чтобы наглядно было видно, что и как
// рисуется поверх. Порядок слоёв (z-index, снизу вверх): красный фон
// (-2) → видео, если загружено (-1) → звуковая волна (0, см.
// soundWave.js — теперь БЕЗ своего непрозрачного фона совсем) → слой
// размытия контура персонажа (1) → сам персонаж (2, см. scene.js).
const diagnosticRedBgEl = document.createElement("div");
diagnosticRedBgEl.style.position = "absolute";
// На 1px больше рамки со всех сторон — та же защита от субпиксельного
// зазора, что и у видео ниже (см. подробный комментарий там). Красный
// слой САМЫЙ нижний — если бы у него был свой зазор, сквозь него было
// бы видно уже фон страницы позади #scene-container, не просто другой
// слой сцены.
diagnosticRedBgEl.style.top = "-1px";
diagnosticRedBgEl.style.left = "-1px";
diagnosticRedBgEl.style.width = "calc(100% + 2px)";
diagnosticRedBgEl.style.height = "calc(100% + 2px)";
diagnosticRedBgEl.style.background = "red";
diagnosticRedBgEl.style.zIndex = "-2";
diagnosticRedBgEl.style.borderRadius = "inherit";
container.appendChild(diagnosticRedBgEl);
...
videoBgFileInput.addEventListener("change", () => {
  ...
  // дефолтное видео/красный слой, если были активны, чтобы не
  // накладывались друг на друга (одинаковый z-index).
  defaultVideoBgEl.style.display = "none";
  defaultVideoBgEl.pause();
  diagnosticRedBgEl.style.display = "none";
...
// ВРЕМЕННО расширено до 12 режимов по кругу (было 4) — "blur" (размытие,
// по умолчанию) → "normal" (чёткие столбики) → "red" (красный
// диагностический слой, волна не видна) → 9 дефолтных видео по очереди
// (тоже волна не видна — по договорённости волна поверх видео больше не
// нужна, не понравилось как смотрится) → снова "blur"... Значения ниже —
// финальные, подобраны и присланы пользователем через временную панель
// настройки (см. её код чуть выше, сейчас спрятана display:none, не
// удалена).
...
const MODE_SETTINGS = {
  ...
  // "red"/"video_N" ничего из этого не используют — просто прячут волну целиком.
};
...
function applyWaveMode(mode) {
  ...
  // Красный слой, дефолтное фоновое видео и кастомное загруженное видео
  // (кнопка 🎬) — все взаимоисключающие с волной и друг с другом
  ...
  soundWave.setVisible(false); // и "red", и "video_N" — волна не видна

  if (mode === "red") {
    diagnosticRedBgEl.style.display = "block";
    return;
  }
