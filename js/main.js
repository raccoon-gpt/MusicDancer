...
function renderLoop() {
  ...
  let beatStrengthThisFrame = 0; // во сколько раз бас СЕЙЧАС выше адаптивного порога beat-детектора (см. beatDetector.js) — непрерывное число, не только true/false на сам момент удара; для пульсации дискошара в такт песне (см. discoLights.js)
  // ВРЕМЕННО (A/B-тест "варианта 1", см. fluxBeatDetector выше) — та же
  // идея, что и beatStrengthThisFrame, но посчитанная от
  // analyzer.getSpectralFlux() вместо features.bass. Какое из двух
  // реально уходит в discoLights.update() ниже — решает
  // ballPulseSource (переключается кнопкой, см.
  // setupBallPulseSourceToggle).
  ...
  if (analyzer && musicIsPlaying) {
    ...
    const { strength: fluxStrength } = fluxBeatDetector.update(flux);
  ...
  // setupBallPulseSourceToggle), discoLights.js сам по себе не знает про
  // выбор источника — просто получает уже выбранное число четвёртым
  // аргументом, как и раньше.
  const ballPulseStrength = ballPulseSource === "flux" ? fluxStrengthThisFrame : beatStrengthThisFrame;
  discoLights.update(delta, intensityThisFrame, strongBeatThisFrame, ballPulseStrength);
