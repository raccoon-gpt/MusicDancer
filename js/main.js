...
function createSteadyTempoTracker({
  ...
  function update(delta, bassBeatDetected, intensity, nowMs = performance.now()) {
    ...
    // временно "соврать" в сторону быстрого темпа — именно это и
    // вызывало "танцует быстрее первые 10-15 секунд на медляке" (см.
    // обсуждение с пользователем): по умолчанию период стартует с
    // "нейтральных" 120 BPM, и на медленном треке уходит несколько
    // секунд + собранных ударов, прежде чем оценка реально успевает
    // доехать до истинного (большего) периода.
    ...
    phase += delta / effectivePeriodSec;
    if (phase >= 1) {
