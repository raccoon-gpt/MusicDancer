...
function setSheetFraction(fraction, { animate = false } = {}) {
  ...
  // Показ/скрытие панели танцев/названия/транспорта — класс .sheet-open
  // (см. style.css) теперь переключается ПРЯМО ЗДЕСЬ, от самого
  // непрерывного fraction, а не только в дискретных openPlaylistSheet()/
  // closePlaylistSheet() (те раньше вызывались ТОЛЬКО по завершении
  // перетаскивания — во время самого процесса ручного перетаскивания
  // пальцем класс вообще не трогался ни разу, весь путь). Из-за этого
  // при МЕДЛЕННОМ перетаскивании старая панель (танцы/прогресс-бар)
  // оставалась полностью видимой ВЕСЬ процесс, одновременно с уже
  // показывающимся содержимым самой шторки снизу — визуально
  // накладывались друг на друга. Порог 0.5 — по факту это ровно
  // середина пути, разумная точка переключения.
  mobileTopArea.classList.toggle("sheet-open", clamped > 0.5);
}
