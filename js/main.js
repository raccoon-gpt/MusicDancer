...
const danceControlsEl = document.getElementById("dance-controls");
const playerTransportEl = document.getElementById("player-transport");
const trackInfoMobileEl = document.getElementById("track-info-mobile");
...
function setSheetFraction(fraction, { animate = false } = {}) {
  ...
  // Панель танцев/название/строка транспорта — ПЛАВНОЕ схлопывание
  // (opacity + max-height), синхронно с тем же fraction, а не дискретный
  // display:none↔flex. Пробовали и мгновенное переключение в начале, и
  // отложенное до конца анимации, и по порогу 0.5 — КАЖДЫЙ вариант давал
  // свой рывок в какой-то момент, потому что display — не то, что можно
  // анимировать плавно, только "есть/нет". max-height/opacity —
  // непрерывные величины, дискретному рывку взяться неоткуда ни при
  // быстром клике, ни при медленном перетаскивании пальцем — оба
  // сценария используют один и тот же путь кода.
  //
  // ВАЖНО: #dance-controls САМ схлопывать нельзя — у него
  // position:relative, он служит "якорем" для #avatar-picker (пикер
  // персонажей открывается ВВЕРХ, за пределы своего блока). overflow:
  // hidden на #dance-controls обрезал бы этот пикер даже при полностью
  // открытой сцене. Схлопываем вместо этого #character-stack — это
  // отдельный СИБЛИНГ #avatar-picker внутри dance-controls (не
  // родитель), клип по нему пикер вообще не задевает. Паддинг самого
  // #dance-controls анимируем отдельно, без overflow — числовое
  // значение, обрезка тут и не нужна.
  //
  // el.scrollHeight читаем СВЕЖИМ каждый раз (не кэшируем один раз) —
  // естественная высота может меняться (например, панель танцев в
  // дуэте — два ряда кнопок вместо одного, а не фиксированная величина).
  [characterStackEl, playerTransportEl, trackInfoMobileEl].forEach((el) => {
    if (!el) return;
    el.style.transition = animate
      ? "opacity 0.25s ease, max-height 0.25s ease, margin 0.25s ease, padding 0.25s ease"
      : "none";
    el.style.opacity = String(1 - clamped);
    if (clamped <= 0) {
      // Полностью открыто (сцена не поджата) — снимаем max-height/
      // overflow СОВСЕМ, а не просто выставляем "большое число". Раньше
      // числовое значение выставлялось ОДИН РАЗ, в момент последнего
      // движения шторки, и замирало — если контент менялся ПОСЛЕ этого
      // (например, загрузился трек с более длинным названием, а шторку
      // с тех пор не трогали) — старое, уже маленькое значение обрезало
      // новый, более высокий контент, хотя место на экране было. Заодно
      // снятие overflow:hidden возвращает overflow:visible — иначе даже
      // в состоянии покоя обрезалось бы, например, свечение (box-shadow)
      // кнопки play внутри #player-transport.
      el.style.maxHeight = "";
      el.style.overflow = "";
    } else {
      el.style.overflow = "hidden";
      el.style.maxHeight = `${(1 - clamped) * el.scrollHeight}px`;
    }
  });
  danceControlsEl.style.transition = animate ? "padding-top 0.25s ease, padding-bottom 0.25s ease" : "none";
  danceControlsEl.style.paddingTop = `${(1 - clamped) * 10}px`;
  danceControlsEl.style.paddingBottom = `${(1 - clamped) * 10}px`;
}
...
function openPlaylistSheet() {
  ...
  setSheetFraction(1, { animate: true }); // .sheet-open переключается ВНУТРИ этой функции, при пересечении fraction>0.5 — см. подробный комментарий там
...
function closePlaylistSheet() {
  ...
  setSheetFraction(0, { animate: true }); // .sheet-open переключается ВНУТРИ этой функции — см. подробный комментарий там
