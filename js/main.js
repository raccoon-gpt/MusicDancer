...
// container и camera нужны звёздному взрыву из-за шара (2D canvas +
// проекция позиции шара на экран, см. createBallStarBurst в
// discoLights.js) — раньше модулю хватало одной сцены, потому что весь
// диско-свет/шар были честными 3D-объектами.
...
function renderLoop() {
  ...
  // Звёздный взрыв из-за шара (см. createBallStarBurst в discoLights.js)
  // — те же самые аудио-реактивные входы, что уже собраны для
  // starTunnel.update() чуть выше, просто вторым, отдельным вызовом (у
  // discoLights.js свой собственный enabled-флаг — если диско-шар сейчас
  // выключен, updateBallStars() ничего не делает и не рисует, см.
  // discoLights.js).
  discoLights.updateBallStars(delta, featuresThisFrame, strongBeatThisFrame, beatThisFrame, riseRateThisFrame);
