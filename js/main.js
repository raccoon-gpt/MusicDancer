...
(function setupCharacterColorControls() {
  ...
  applyEdgeBlur(2); // финальное значение по умолчанию, применяется сразу при загрузке
  ...
  addSlider("Размытие краёв", 0, 40, 1, 2, (v) => applyEdgeBlur(v));
...
function loadTrackAtIndex(index) {
  ...
  trackTitleTextEl.textContent = title;
  trackArtistEl.textContent = artist;
  trackInfoMobileEl.classList.add("has-track"); // прячем пустой ">" до первого выбранного трека — см. style.css
...
playlistDragHandle.addEventListener("pointermove", (e) => {
  ...
  setSheetFraction(sheetDragStartFraction + deltaFraction, { animate: false });
});
