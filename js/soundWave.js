...
export function createSoundWave(container) {
  ...
  function draw() {
    // Раньше здесь была непрозрачная заливка (BG_COLOR, #0d0d0d) — теперь
    // убрана СОВСЕМ, не только условно. Роль "нижнего фона по умолчанию"
    // теперь у отдельного постоянного красного слоя в main.js (диагностика
    // многослойности), который никогда не отключается — волне больше не
    // нужно бояться "дыры", если её фон убрать, всегда есть что показать
    // позади неё.
