...
const INTENSITY_ACTIVE_DOWN = 0.09; // active → calm, когда энергия падает —
// снижено с 0.12 по просьбе (реже уходить в idle): нужен ещё более тихий
// момент, чтобы вообще начать рассматривать переход в idle, а не обычный
...
const DEFAULT_TUNING = {
  ...
  // в танец.
  calmConfirmSeconds: 1.2, // повышено с 0.8 по просьбе (реже уходить в
  // idle) — тихий участок должен продержаться ещё дольше, прежде чем
  // персонаж реально перейдёт в простой, а не просто на мгновение
  // просядет громкость.
};
