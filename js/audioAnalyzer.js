...
export function createAudioAnalyzer(audioElement) {
  ...
  // Снимок freqData ПРОШЛОГО кадра — нужен только для getSpectralFlux()
  // ниже (спектральный поток сравнивает текущий спектр с предыдущим, не
  // абсолютные уровни сами по себе). null, пока не было ни одного
  // кадра — тогда сравнивать не с чем.
  let previousFreqData = null;
  ...
  /**
   * Spectral flux ("вариант 1" из обсуждения с пользователем) —
   * классический DSP-приём определения "нового звукового события"
   * (onset detection), НЕ ML-разделение источников. Идея: удар
   * (кик/снейр/хай-хэт) даёт РЕЗКИЙ одновременный подъём энергии сразу
   * по многим частотным бинам ("широкополосный щелчок"), а мелодия/бас-
   * линия/вокал меняются от кадра к кадру заметно плавнее. Считаем
   * сумму ТОЛЬКО положительных приростов энергии по бинам между этим
   * кадром и прошлым (падения энергии игнорируем — затухание ноты не
   * связано с новым ударом, только НАРАСТАНИЕ). Это отличается от
   * getFeatures().bass — тот смотрит на АБСОЛЮТНЫЙ уровень низких
   * частот, поэтому не отличает "чистый кик" от "кик поверх сустейнящей
   * басовой партии" (см. обсуждение — из-за этого сейчас изолированная
   * дорожка ударных пользователя даёт более чистый бит, чем полный
   * трек). Spectral flux вместо этого реагирует на САМ ПЕРЕПАД, поэтому
   * устойчивее к постоянному фоновому басу под ударными.
   *
   * ВАЖНО: переиспользует freqData этого же кадра (как и
   * getSpectrumBars выше) — вызывать ПОСЛЕ getFeatures() в том же
   * кадре, отдельного analyser.getByteFrequencyData() здесь нет.
   *
   * @param {number} maxHz - верхняя граница диапазона, в котором считаем
   *   перепад. По умолчанию 8000 — сознательно НЕ весь спектр:
   *   самые верхние частоты (выше ~8кГц) у большинства треков — это
   *   более-менее ровный шум/шипение тарелок/сведение, который скорее
   *   добавляет дрожащий "постоянный шум" в сумму разностей, чем
   *   помогает отличить именно НОВЫЙ удар. Диапазон подобран на глаз,
   *   не измерялся точно — если после теста окажется, что хай-хэты не
   *   ловятся, есть смысл поднять до 10-12кГц.
   * @returns {number} 0..1 — нормированная сумма положительных приростов
   *   (математически не может выйти за 0..1 — максимум достигается,
   *   только если КАЖДЫЙ бин в диапазоне разом прыгнул с 0 до предела
   *   256, что на реальном сигнале не происходит, так что реальные
   *   значения обычно заметно ниже 1 — это ожидаемо и нормально, тот же
   *   принцип, что и у сырого bass/mid до всякой донастройки).
   */
  function getSpectralFlux(maxHz = 8000) {
    const toBin = Math.min(freqData.length - 1, Math.ceil(maxHz / hzPerBin));

    if (!previousFreqData) {
      // Первый вызов — эталона для сравнения ещё нет.
      previousFreqData = new Uint8Array(freqData);
      return 0;
    }

    let sum = 0;
    for (let i = 0; i <= toBin; i++) {
      const diff = freqData[i] - previousFreqData[i];
      if (diff > 0) sum += diff;
    }
    previousFreqData.set(freqData); // этот кадр становится "предыдущим" для следующего вызова

    const binCount = toBin + 1;
    return sum / (binCount * 255);
  }

  return { audioCtx, resume, getFeatures, getSpectrumBars, getSpectralFlux };
}
