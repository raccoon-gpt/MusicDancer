...
function createBallShockwave(container) {
  ...
  const ROTATION_SPEED = 0.45; // радиан/сек — было 0.15, увеличено в 3 раза по просьбе пользователя (тест, можно вернуть обратно)
  const BREATH_PERIOD_SEC = 3; // секунд на полный цикл 100%→110%→100% — было 6 (по 3с на этап), стало 3 (по 1.5с на этап), по просьбе пользователя (тест, можно вернуть обратно)
