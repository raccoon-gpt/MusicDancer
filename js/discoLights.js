...
export function createDiscoLights(scene) {
  ...
  function update(delta, intensity = 0, strongBeat = false) {
    ...
    fixtures.forEach(({ light, helper, baseAngle, speed }) => {
      ...
      // Радиус СИЛЬНО уменьшен (было 2.4) — источник висит высоко над
      // центром (0, 3.2, 0), а персонаж гораздо компактнее широкого
      // круга: луч, идущий от источника к далёкой точке цели, у самого
      // верха проходит близко к персонажу, но по мере снижения к цели
      // расходится наружу, уходя мимо тела ближе к ногам — визуально
      // "лучи кружат вокруг персонажа, а не проходят сквозь него".
      // Меньший радиус держит луч плотнее к персонажу на всей высоте.
      const radius = 0.9;
