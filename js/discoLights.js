...
export function createDiscoLights(scene) {
  ...
  // "трек только должен диктовать КОГДА 95, 100 или 91" — а не какое
  // конкретно промежуточное значение.
  //
  // СЕЙЧАС — РОВНО ТРИ фиксированных состояния масштаба: 100% (покой),
  // 97% (обычный удар), 85% ("супер-сильный" удар, ×1.4 от порога — см.
  // main.js). Никаких других чисел, кроме
  // этих трёх, нигде не участвует как ЦЕЛЬ. Трек (ballBeat/ballStrongBeat
  // — булевы события "случился обычный/сильный удар в этом кадре", не
  // число) решает ТОЛЬКО, к какой из этих трёх целей сейчас ехать и
  // когда переключаться. Сам проезд между текущим значением и целью —
  // отдельный, чисто механический процесс с ФИКСИРОВАННОЙ скоростью
  // (BALL_PULSE_EASE_TAU ниже) — не имеет никакого отношения к
  // силе/громкости удара, только ко времени. Именно поэтому шар и
  // проходит через промежуточные проценты ВО ВРЕМЯ самого перехода
  // (99%, 97% и т.д. по пути) — это неизбежная механика плавной
  // анимации между двумя точками, а не то, что трек "диктует" эти
  // числа.
  const BALL_PULSE_LEVEL_NORMAL = 0.97; // цель на обычном ударе
  const BALL_PULSE_LEVEL_STRONG = 0.85; // цель на "супер-сильном" ударе (×1.4 от порога, см. main.js)
  // Сколько секунд после удара держим цель внизу (97% или 85%), прежде
  // чем отпустить её обратно к 100% (если за это время не пришёл
  // следующий удар — тогда цель просто обновится заново на новый удар,
  // см. update() ниже). Короткое и ФИКСИРОВАННОЕ значение — не
  ...
  // темп перехода что для 97%, что для 85%, что для возврата к 100%.
  const BALL_PULSE_EASE_TAU = 0.06;

  let ballPulseTarget = 1; // текущая ЦЕЛЬ — строго одно из трёх: 1 / BALL_PULSE_LEVEL_NORMAL / BALL_PULSE_LEVEL_STRONG
   ...
   * @param {boolean} ballBeat - обычный удар (см. main.js — источник выбирается кнопкой A/B-теста: bass или spectral flux) — переключает цель пульсации шара на 97%
   * @param {boolean} ballStrongBeat - сильный удар из ТОГО ЖЕ выбранного источника, что и ballBeat — переключает цель пульсации шара на 85% (приоритетнее ballBeat в тот же кадр)
   */
  function update(delta, intensity = 0, strongBeat = false, ballBeat = false, ballStrongBeat = false) {
    if (!enabled) return;
    time += delta;
    ...
    // Три строго фиксированных состояния — см. подробное объяснение
    // выше у BALL_PULSE_LEVEL_NORMAL/BALL_PULSE_LEVEL_STRONG.
    // ballStrongBeat проверяется ПЕРВЫМ — если в этом кадре одновременно
    // случились и обычный, и сильный удар (а strongBeat технически и
    // означает "это ballBeat, который вдобавок оказался достаточно
    // сильным", см. beatDetector.js), должна победить более глубокая
    // цель, а не мельче.
    if (ballStrongBeat) {
      ballPulseTarget = BALL_PULSE_LEVEL_STRONG;
