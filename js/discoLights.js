...
export function createDiscoLights(scene, container, camera) {
  ...
  // Пульсация шара — ПОСЛЕ двух неудачных попыток через mid (см. историю
  // правок — сперва простой гейн от абсолютного mid упирался в потолок,
  // потом реакция на "всплеск относительно недавней базы" всё равно не
  // попадала в такт и продолжала слегка дрожать на затухании) решили
  // взять ТЕ ЖЕ строительные блоки, что уже держат весь остальной
  // диско-свет и эквалайзер (soundWave.js), а не изобретать новую
  // сглаживающую схему специально под шар:
  //  - intensity — тот же параметр, что уже даёт базовую яркость
  //    прожекторов (baseIntensity ниже) — AGC-нормализованная
  //    "энергичность" (см. createIntensityTracker в audioAnalyzer.js):
  //    сама собой стремится к 0, когда мелодия затихает (это не сырая
  //    громкость, а её отношение к диапазону ИМЕННО этого трека), и
  //    сама собой поднимается в энергичных местах — то есть именно то
  //    "не пульсировать на затухании" поведение, которое просили;
  //  - beatPulse (уже посчитан чуть выше для ламп шара/прожекторов) —
  //    мгновенный скачок ровно в момент strongBeat с быстрым затуханием,
  //    та же механика, что и beatPunch в soundWave.js (там тоже "щелчок"
  //    строго на удар, а не сглаженная погоня за спектром) — это и даёт
  //    отклик именно "в такт", без задержки от собственного сглаживания.
  // Ничего больше не сглаживаем отдельно для пульсации — оба слагаемых
  // уже сглажены на своём уровне (AGC и экспоненциальное затухание
  // соответственно), увеличивать это в третий раз только добавило бы
  // задержку.
  const BALL_PULSE_STRENGTH = 0.22; // на сколько максимум "раздувается" шар на пике (интенсивный момент + сильный удар одновременно)
  ...
  /**
   * @param {number} delta - секунды с прошлого кадра
   * @param {number} intensity - 0..1, "энергичность" текущего момента музыки (см. createIntensityTracker в audioAnalyzer.js) — управляет базовой яркостью и (теперь) базовым уровнем пульсации шара
   * @param {boolean} strongBeat - был ли в этом кадре сильный удар — короткая вспышка яркости и (теперь) пульсации шара, см. beatPulse
   */
  function update(delta, intensity = 0, strongBeat = false) {
    ...
    // персонажа, и лучей вокруг шара, и (теперь) пульсации размера самого
    // шара. Вращение шара — НЕ использует это (см. ниже, постоянная
    // скорость по просьбе).
    if (strongBeat) beatPulse = 1;
    beatPulse *= Math.pow(0.02, delta); // плавное, но довольно быстрое затухание вспышки

    // Пульсация размера — см. подробное объяснение у BALL_PULSE_STRENGTH
    // выше: intensity даёт плавный "энергичный/спокойный" фон (и уходит
    // в 0 на затухании), beatPulse добавляет резкий отклик ровно в такт
    // удара поверх него. clamp на 1 — просто защита от переполнения
    // (сумма двух вкладов теоретически может выйти за пределы 0..1).
    const ballPulse = Math.min(1, intensity * 0.5 + beatPulse * 0.9);
    ...
    discoBall.scale.setScalar(baseBallScale * (1 + ballPulse * BALL_PULSE_STRENGTH));
