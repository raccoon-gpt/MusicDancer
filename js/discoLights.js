...
function createBallStarBurst(container) {
  ...
  function update(delta, features, strongBeat, beat, riseRate, origin) {
    ...
    let speed; // px/сек
    if (BALL_STAR_UNLIMITED_RADIUS) {
      // В unlimited-режиме радиус сопоставим по масштабу с полноэкранным
      // туннелем — берём ТУ ЖЕ САМУЮ формулу, что и в starTunnel.js
      // (абсолютные px/сек), а не пересчитанную "долю радиуса в
      // секунду" ниже: та формула была откалибрована под маленький
      // радиус вокруг шара и на полноэкранном масштабе оказалась
      // завышена в 8-12 раз — звёзды летели заметно быстрее и
      // перезапускались заметно чаще, чем в оригинале (пользователь
      // заметил на глаз "паровозик" — на самом деле это была просто
      // намного более высокая частота новых звёзд, не баг отрисовки).
      const baseSpeed = 40 + volume * 220 + riseBoost * 140;
      speed = baseSpeed * (1 + beatPulse * 2.2);
    } else {
      // Ограниченный режим (радиус завязан на видимый размер шара) —
      // тут скорость намеренно выражена в ДОЛЯХ РАДИУСА в секунду, не в
      // пикселях напрямую, чтобы поведение оставалось одинаковым
      // независимо от текущего видимого размера шара (соло/дуэт,
      // пульсация от удара — origin.radiusPx каждый кадр может быть
      // разным, а абсолютная скорость "потерялась" бы при масштабе в
      // несколько раз меньше экрана).
      const baseSpeedFraction = 0.5 + volume * 2.4 + riseBoost * 1.5;
      const speedFraction = baseSpeedFraction * (1 + beatPulse * 2.2);
      speed = speedFraction * maxRadiusPx;
    }
