...
export function createDiscoLights(scene, container, camera) {
  ...
  let smoothedMid = 0;
  // Сырой mid из audioAnalyzer.js на реальных треках обычно сидит в
  // районе ~0.1-0.45 (редко подбирается к 1) — если умножать силу
  // пульсации прямо на такое сырое значение, эффект получается
  // мизерным (доли процента размера, "на миллиметр", как справедливо
  // заметил пользователь), хотя сама шкала mid на глаз скачет очень
  // активно. MID_GAIN растягивает typical-диапазон ближе к 0..1 (тот же
  // приём, что и volumeScale в createIntensityTracker в
  // audioAnalyzer.js), ПОСЛЕ чего уже применяется MID_PULSE_STRENGTH —
  // сила пульсации подняли с 0.1 (10%) до 0.35 (35%), должно быть
  // хорошо заметно, не "на миллиметр".
  const MID_GAIN = 1.8;
  const MID_PULSE_STRENGTH = 0.35; // на сколько максимум "раздувается" шар при (растянутом) mid=1
  ...
  function update(delta, intensity = 0, strongBeat = false, mid = 0) {
    ...
    const gainedMid = Math.min(1, mid * MID_GAIN); // растяжка диапазона, см. комментарий у MID_GAIN выше; clamp — чтобы громкие треки не улетали за 1 и не давали scale меньше нуля/отрицательный смысл
    smoothedMid += (gainedMid - smoothedMid) * midEase;
