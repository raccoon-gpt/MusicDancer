...
export function createDiscoLights(scene) {
  ...
  // 95% (обычный удар), 85% ("супер-сильный" удар, ×2 от порога — см.
  ...
  // числа.
  const BALL_PULSE_LEVEL_NORMAL = 0.95; // цель на обычном ударе
  const BALL_PULSE_LEVEL_STRONG = 0.85; // цель на "супер-сильном" ударе (×2 от порога, см. main.js)
  // Сколько секунд после удара держим цель внизу (95% или 85%), прежде
  ...
  // темп перехода что для 95%, что для 85%, что для возврата к 100%.
   ...
   * @param {boolean} ballBeat - обычный удар (см. main.js — источник выбирается кнопкой A/B-теста: bass или spectral flux) — переключает цель пульсации шара на 95%
