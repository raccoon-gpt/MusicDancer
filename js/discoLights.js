...
function createBallShockwave(container) {
  ...
  const ROTATION_SPEED = 0.45; // радиан/сек — постоянное вращение картинки
  const BREATH_PERIOD_SEC = 3; // секунд на полный цикл 100%→140%→100% — по 1.5с на каждый этап
  const BREATH_AMPLITUDE = 0.4; // 40% — было 10%, увеличено по просьбе пользователя
  ...
  function update(delta, origin, shouldSpawn) {
    ...
    for (let i = waves.length - 1; i >= 0; i--) {
      ...
      // WAVE_BASE_OPACITY — по просьбе пользователя: без этого множителя
      // альфа на самом пике (t≈0, flickerFactor=1) доходила бы до 100%
      // (полностью непрозрачно) — теперь потолок 90%, даже в момент
      // максимальной видимости волна не абсолютно непрозрачна.
      const alpha = Math.pow(Math.max(0, 1 - t), 1.4) * flickerFactor * WAVE_BASE_OPACITY;
