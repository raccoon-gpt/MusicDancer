...
export function createDiscoLights(scene, container, camera) {
  ...
  // Пульсация от mid — раньше (см. историю правок) пробовали просто
  // растянуть гейном (MID_GAIN) абсолютный уровень mid к 0..1 — не
  // сработало: у реальных треков mid почти всё время сидит достаточно
  // высоко (не пусто-тихо), гейн+clamp почти сразу упирался в потолок
  // 1 — шар распухал до максимума в первые секунды и дальше просто
  // мелко дрожал у этого потолка (ровно то, что заметил пользователь).
  //
  // ПОДХОД СЕЙЧАС — реагируем не на абсолютный уровень mid, а на
  // ВСПЛЕСК относительно недавнего "обычного" уровня ЭТОГО КОНКРЕТНОГО
  // трека: считаем ДВА сглаженных значения с разной скоростью —
  // medium-term "база" (тянется медленно, секунды — это и есть текущий
  // средний уровень громкости mid в этом месте трека) и быстрый "сейчас"
  // (тянется быстро, как раньше). Шар раздувается только когда "сейчас"
  // заметно ВЫШЕ своей же недавней базы — то есть именно на реальных
  // всплесках энергии, а не сидит раздутым постоянно. Работает
  // одинаково и на тихих, и на громких треках, потому что база
  // подстраивается под каждый сама.
  let midBaseline = 0;
  let midFast = 0;
  const MID_DEVIATION_GAIN = 6; // во сколько раз усиливаем всплеск (fast - baseline) перед тем, как превратить его в 0..1
  const MID_PULSE_STRENGTH = 0.3; // на сколько максимум "раздувается" шар на пике всплеска (30% от базового размера)
   ...
   * @param {number} mid - 0..1, средние частоты (см. features.mid в audioAnalyzer.js/main.js) — лёгкая пульсация размера шара на всплесках mid относительно недавней базы, см. комментарий у midBaseline/midFast выше
   */
  function update(delta, intensity = 0, strongBeat = false, mid = 0) {
    ...
    // "База" — медленно (секунды) подстраивается под текущий обычный
    // уровень mid этого трека/участка. "Сейчас" — быстро следует за
    // реальным сигналом, как и раньше. 0.5 за секунду (для базы) — это
    // сильное сглаживание, ~1.4с до заметного отклика; 0.001 за секунду
    // (для "сейчас") — почти "сырое" значение, тот же темп, что и
    // раньше.
    const midBaselineEase = 1 - Math.pow(0.5, delta);
    const midFastEase = 1 - Math.pow(0.001, delta);
    midBaseline += (mid - midBaseline) * midBaselineEase;
    midFast += (mid - midFast) * midFastEase;

    // Только ПОЛОЖИТЕЛЬНОЕ отклонение (сейчас громче своей недавней
    // базы) раздувает шар — тихие моменты не должны его сжимать ниже
    // обычного размера, только "вспышки" энергии должны его увеличивать.
    const midBurst = Math.max(0, midFast - midBaseline);
    const midPulse = Math.min(1, midBurst * MID_DEVIATION_GAIN);
    ...
    discoBall.scale.setScalar(baseBallScale * (1 + midPulse * MID_PULSE_STRENGTH));
