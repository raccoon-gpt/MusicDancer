...
function createDiscoBall(scene) {
  ...
  const material = new THREE.MeshStandardMaterial({
    color: 0xd8d8d8,
    // Металличность снижена с 0.95 — у материалов, близких к "чистому
    // металлу", корректное отражение по-хорошему требует environment map
    // (карту окружения сцены); без неё такой материал в Three.js обычно
    // выглядит плоско-серым от прямого света, а не даёт яркие точечные
    // блики — у настоящих металлов почти нет обычного рассеянного
    // отражения, только зеркальное, и без карты окружения рендереру
    // особо нечего зеркалить. Более низкая металличность ведёт себя
    // ближе к "глянцевому, но не чистому металлу" — заметнее откликается
    // именно на прямые точечные источники, без карты окружения.
    metalness: 0.6,
    roughness: 0.18,
    flatShading: true, // подчёркивает отдельные грани, а не сглаживает их — важно именно для "дискошарного" вида
    // Пробовали добавить emissive (лёгкое собственное свечение) —
    // откатили: оно красит ВСЕ грани равномерно, независимо от того,
    // попадает туда луч конкретного прожектора или нет — стирает
    // контраст между освещёнными и неосвещёнными гранями, то есть как
    // раз то, что и даёт "искрящийся" вид дискошара. Шар начинал
    // выглядеть просто ровно-серым, а не переливающимся точечными
    // бликами. Оставляем чисто отражающий материал, без своего
    // свечения — блеск должен идти ТОЛЬКО от реальных источников света.
  });
