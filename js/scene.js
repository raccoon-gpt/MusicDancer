...
export function createScene(container) {
  ...
  function resize() {
    // getBoundingClientRect() + Math.ceil (округление ВВЕРХ), не
    // container.clientWidth/clientHeight (те округляют к БЛИЖАЙШЕМУ
    // целому, в том числе иногда ВНИЗ). Контейнер может получить дробную
    // высоту в пикселях — например, кнопка масштаба сцены считает высоту
    // как naturalSize*0.8, что для нецелого naturalSize даёт нецелый
    // результат (274.4px и т.п.). Красный диагностический слой/видео-фон
    // (см. main.js) идут через CSS width/height:100% — те отражают ТОЧНЫЙ
    // дробный размер честно. А canvas.width/height — целые пиксели по
    // определению; если округлить к БЛИЖАЙШЕМУ (а не вверх), канвас на
    // конкретных значениях масштаба оказывался на доли пикселя МЕНЬШЕ
    // контейнера — и в эту щель снизу/по углам просвечивал слой позади
    // (заметно стало только сейчас, когда убрали непрозрачный фон волны,
    // который раньше эту щель случайно маскировал). Округление ВВЕРХ
    // гарантирует, что канвас всегда МИНИМУМ равен контейнеру, никогда не
    // меньше — на глаз лишняя доля пикселя незаметна, а щели взяться
    // неоткуда.
      ...
      (fbx) => {
        ...
        fbx.traverse((node) => {
          if (node.isMesh) {
            node.castShadow = true;
            node.receiveShadow = true;
            // Отключаем frustum culling — известная частая проблема со
            // скелетными анимированными персонажами в Three.js:
            // bounding-сфера, по которой движок решает "виден ли объект
            // в кадре камеры", по умолчанию считается ОДИН РАЗ по
            // исходной позе геометрии (bind pose) и не обновляется
            // автоматически при движении костей во время анимации/танца.
            // При определённых углах камеры (особенно вид строго сверху)
            // движок может ошибочно решить, что персонаж "вне кадра", и
            // просто пропустить его отрисовку — хотя фактически, после
            // анимации, он там есть. Цена отключения — небольшая (эта
            // сцена и так рисует буквально один-два персонажа за раз, не
            // тысячи объектов), а надёжность важнее.
